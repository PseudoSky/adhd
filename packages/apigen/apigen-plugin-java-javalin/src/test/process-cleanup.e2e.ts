 ...
 * runs in its lane like every other live test in this package.
 *
 * RESOURCE LANE: proc (real child-process spawn + SIGTERM + `pgrep`/`ps`).
 * This file runs ONLY under `nx run apigen-plugin-java-javalin:e2e` (see the
 * sibling `vitest.e2e.config.ts`) — never under `nx affected -t test` or the
 * pre-commit / pre-push hooks, which run only `*.spec.ts`. The cheap
 * `process-cleanup.spec.ts` at the original path holds the mocked-placeholder
 * `it.todo` inventory of this case.
 */
