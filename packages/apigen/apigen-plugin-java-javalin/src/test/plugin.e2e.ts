/**
 * java-javalin plugin E2E — drives a LIVE two-phase-spawn pipeline: a REAL
 ...
 *
 * RESOURCE LANE: proc (subprocess + real bound port). This file runs ONLY
 * under `nx run apigen-plugin-java-javalin:e2e` (see the sibling
 * `vitest.e2e.config.ts`) — never under `nx affected -t test` or the
 * pre-commit / pre-push hooks, which run only `*.spec.ts`. The cheap
 * `plugin.spec.ts` at the original path holds the mocked-placeholder
 * `it.todo` inventory of these cases plus the pure (non-spawning)
 * `resolveJavaPkgDir` / `selectFatJar` tests that need no resource lane.
 *
