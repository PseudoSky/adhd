...
describe('plugin generator', () => {
  ...
  it('creates all expected files for a plugin', async () => {
    ...
    await pluginGenerator(tree, { name: 'python-grpc' });
    expect(
      tree.exists('packages/apigen/plugins/python-grpc/project.json')
    ).toBe(true);
    expect(
      tree.exists('packages/apigen/plugins/python-grpc/src/lib/plugin.ts')
    ).toBe(true);
    expect(
      tree.exists('packages/apigen/plugins/python-grpc/src/index.ts')
    ).toBe(true);
    expect(
      tree.exists('packages/apigen/plugins/python-grpc/src/test/plugin.spec.ts')
    ).toBe(true);
    expect(
      tree.exists('packages/apigen/plugins/python-grpc/package.json')
    ).toBe(true);
  });
  ...
  it('updates tsconfig.base.json with the new plugin path', async () => {
    ...
    expect(
      tsconfig.compilerOptions.paths['@adhd/apigen-plugin-tsconfig-test']
  ...
  it('uses custom directory when provided', async () => {
    ...
    await pluginGenerator(tree, {
      name: 'my-plugin',
      directory: 'packages/custom/my-plugin',
    });
  ...
  it('default home is packages/apigen/plugins/<name> and project name is apigen-plugin-<name>', async () => {
    ...
    expect(tree.exists('packages/apigen/plugins/my-target/project.json')).toBe(
      true
    );
