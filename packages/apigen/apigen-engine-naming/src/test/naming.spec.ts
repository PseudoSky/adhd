...
describe('toKebab', () => {
  ...
  it('[naming.toKebab.3] three-word segment', () => {
    expect(toKebab(seg('getMyData', ['get', 'my', 'data']))).toBe(
      'get-my-data'
    );
  });
});
...
describe('toSnake', () => {
  ...
  it('[naming.toSnake.3] three words', () => {
    expect(toSnake(seg('getMyData', ['get', 'my', 'data']))).toBe(
      'get_my_data'
    );
  });
});
...
describe('checkCollisions', () => {
  const opA = op('pkg/a', seg('pkg', ['pkg']), [seg('a', ['a'])], false);
  const opB = op('pkg/b', seg('pkg', ['pkg']), [seg('b', ['b'])], true);
  ...
  it('[naming.collision.4] two DISTINCT ids that resolve to the same MCP name → THROWS', () => {
    ...
    expect(() => checkCollisions([collision1, collision2])).toThrow(
      CollisionDetectedError
    );
  });
