 ...
 *
 * RESOURCE LANE: proc. This file runs ONLY under
 * `nx run apigen-engine-runtime:e2e` (see the sibling `vitest.e2e.config.ts`),
 * never under `nx affected -t test` or the pre-commit / pre-push hooks, which
 * run only `*.spec.ts`. It stays `describe.skip`'d (CPU-THRASH-SKIP,
 * owner-requested) exactly as before — the move did NOT un-skip it; the e2e
 * lane is its correct home for when the owner re-enables it. The cheap
 * `isolated-git.spec.ts` at the original path holds the mocked-placeholder
 * `it.todo` inventory of these cases.
 */
