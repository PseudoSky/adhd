 ...
 * `isolated-git.e2e.ts` for the proven escape mechanism and its fix).
 *
 * RESOURCE LANE: proc (real bound HTTP port + real `git apply`). This file
 * runs ONLY under `nx run apigen-engine-runtime:e2e` (see the sibling
 * `vitest.e2e.config.ts`), never under `nx affected -t test` or the
 * pre-commit / pre-push hooks, which run only `*.spec.ts`. Both suites below
 * stay `describe.skip`'d (CPU-THRASH-SKIP, owner-requested) exactly as before
 * — the move did NOT un-skip them; the e2e lane is their correct home for when
 * the owner re-enables them. The cheap `parity-harness.spec.ts` at the original
 * path holds the mocked-placeholder `it.todo` inventory of these cases.
 */
