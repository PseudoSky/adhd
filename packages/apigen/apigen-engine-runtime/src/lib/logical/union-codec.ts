 ...
 */
export function createUnionCodec(
  opts: UnionCodecOptions
  ...
  return {
    ...
    encode(value: object, node: SchemaNode, ctx: TranscodeCtx): Wire {
      ...
      if (!ref) {
        throw new UnionCodecError(
          'E_UNION_UNKNOWN_TAG',
          `[${id}] discriminator tag "${tagStr}" at "${
            ctx.path
          }" is not in the mapping (known: ${Object.keys(mapping).join(', ')}).`
        );
      }
      ...
      return encodeBranch(value, branchNode, {
        ...ctx,
        path: `${ctx.path}[${tagStr}]`,
      });
    },
    ...
    decode(wire: Wire, node: SchemaNode, ctx: TranscodeCtx): object {
      ...
      if (!ref) {
        throw new UnionCodecError(
          'E_UNION_UNKNOWN_TAG',
          `[${id}] discriminator tag "${tagStr}" on the wire at "${
            ctx.path
          }" is not in the mapping (known: ${Object.keys(mapping).join(', ')}).`
        );
      }
      ...
      const branchNode = ctx.resolve(ref);
      return decodeBranch(wire, branchNode, {
        ...ctx,
        path: `${ctx.path}[${tagStr}]`,
      }) as object;
    },
