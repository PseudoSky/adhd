...
function requiredFields(node: SchemaNode): readonly string[] {
  ...
  return Array.isArray(req)
    ? (req.filter((k) => typeof k === 'string') as string[])
    : [];
}
 ...
 */
export function createNominalCodec(
  opts: NominalCodecOptions
): LogicalTypeCodec<object> {
  ...
  return {
    ...
    encode(value: object, node: SchemaNode, ctx: TranscodeCtx): Wire {
      ...
      try {
        ...
        // ── 2. toJSON hint ────────────────────────────────────────────────────
        const tojsonName =
          typeof node[X_APIGEN_TOJSON] === 'string'
            ? (node[X_APIGEN_TOJSON] as string)
            : 'toJSON';
    ...
    decode(wire: Wire, node: SchemaNode, ctx: TranscodeCtx): object {
      ...
      if (ctor) {
        const ctorName =
          typeof node[X_APIGEN_CTOR] === 'string'
            ? (node[X_APIGEN_CTOR] as string)
            : 'fromJSON';
        const reconstructor = (ctor as Record<string, unknown>)[ctorName];
        if (typeof reconstructor === 'function') {
          return (reconstructor as (b: Record<string, unknown>) => object).call(
            ctor,
            bag
          );
        }
