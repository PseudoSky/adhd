...
class Dog {
  ...
  constructor(readonly name: string, readonly breed: string) {}
...
class Cat {
  ...
  constructor(readonly name: string, readonly indoor: boolean) {}
...
function buildRegistry(): LogicalTypeRegistry {
  ...
  registry.register(
    createNominalCodec({ id: 'cli.Dog', schema: DOG_NODE, ctor: Dog as never })
  );
  registry.register(
    createNominalCodec({ id: 'cli.Cat', schema: CAT_NODE, ctor: Cat as never })
  );
  registry.register(
    createUnionCodec({ id: 'cli.Pet', schema: PET_UNION_NODE })
  );
  return registry;
}
...
describe('createUnionCodec', () => {
  ...
  it('does NOT match a plain object node', () => {
    const codec = createUnionCodec({ id: 'cli.Pet', schema: PET_UNION_NODE });
    expect(
      codec.matches({ type: 'object', properties: {} } as SchemaNode)
    ).toBe(false);
  });

  it('does NOT match a node with oneOf but no discriminator', () => {
    const codec = createUnionCodec({ id: 'cli.Pet', schema: PET_UNION_NODE });
    expect(codec.matches({ oneOf: [{ type: 'string' }] } as SchemaNode)).toBe(
      false
    );
  });
  ...
  it('[neg-c] hints-advisory: decoding with x-apigen-logical stripped yields identical result', () => {
    ...
    const wireWithHint = codec.encode(dog, PET_UNION_NODE, ctx) as Record<
      string,
      Wire
    >;
    // Encode against the node WITHOUT the hint (structure only)
    const wireWithoutHint = codec.encode(
      dog,
      PET_UNION_NODE_NO_HINT,
      ctxFor(registry)
    ...
    // Decode with hint
    const decodedWithHint = codec.decode(
      wireWithHint,
      PET_UNION_NODE,
      ctxFor(registry)
    );
    // Decode without hint (structure-only node)
    const decodedWithoutHint = codec.decode(
      wireWithoutHint,
      PET_UNION_NODE_NO_HINT,
      ctxFor(registry)
    );
  ...
  it('[teeth] a schema with an empty mapping always throws E_UNION_UNKNOWN_TAG (proves the test has teeth)', () => {
    ...
    try {
      ...
      throw new Error(
        'expected E_UNION_UNKNOWN_TAG — if this fires, the codec is broken'
      );
    } catch (err) {
      expect(err).toBeInstanceOf(UnionCodecError);
      expect((err as UnionCodecError).code).toBe('E_UNION_UNKNOWN_TAG');
