 ...
 */
export function makeUnavailableError(
  host: string,
  operationId: string,
  reason: string
): ApiError {
 ...
 */
export function makeDeadlineExceededError(
  host: string,
  operationId: string,
  deadlineMs: number
): ApiError {
  ...
  return new ApiError(
    'internal',
    `op '${operationId}' on host '${host}' exceeded ${deadlineMs}ms deadline`,
    detail
  );
}
...
export function isGatewayError(
  ...
  return (
    d.gatewayCode === 'unavailable' || d.gatewayCode === 'deadline_exceeded'
  );
}
...
export function createInProcessHostAdapter(
  ...
  return {
    ...
    async invoke(req, signal) {
      if (!started)
        throw makeUnavailableError(host, req.operation.id, 'not started');
...
export function createGateway(opts: GatewayOptions): Gateway {
  ...
  for (const adapter of opts.adapters) {
    if (hosts.has(adapter.host)) {
      throw new Error(
        `apigen-gateway: duplicate host adapter '${adapter.host}'`
      );
    }
    ...
    async route(req, signal) {
      ...
      if (entry == null) {
        throw makeUnavailableError(
          host,
          req.operation.id,
          'no adapter registered for host'
        );
      }
