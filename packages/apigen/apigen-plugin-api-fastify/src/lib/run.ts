...
export async function run(input: RunInput): Promise<void> {
  ...
  // bypass of auth/logging/validate). `hostInvoke`'s real `InvokeFn` shape
  // already matches `MountHostBridge.invoke`'s duck-typed signature except for
  // needing `operation`/`ctx` filled in per call — done here exactly like
  // `dispatchForPlan`'s own mount-branch adaptation.
