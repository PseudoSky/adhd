...
// Run: npx vitest run src/test/plugin.e2e.ts --config vitest.e2e.config.ts -t "LIVE server"
      ...
      function runGoldenParityCheckInFreshProcess(): void {
        const result = spawnSync(
          ...
          [
            ...
            'run',
            '--config',
            path.join(repoRoot, 'packages/apigen/apigen-plugin-py-flask/vitest.e2e.config.ts'),
            '-t',
