 ...
 * api-express generator spliced a discovered package id straight into JS
...
describe('generate() — hyphenated package id sanitization (unit)', () => {
  it('[hyphen-express.1] sanitizes the import-namespace and fn-table identifiers', () => {
  ...
  it('[hyphen-express.2] sanitizes the identifier at both GET and POST dispatch call sites', () => {
...
describe('generate() — REAL syntax verification via esbuild', () => {
  it('[hyphen-express.5] proves the OLD unsanitized splice was a genuine parse error', () => {
  ...
  it('[hyphen-express.6] the current generate() output parses and transpiles cleanly', () => {
    const { content } = generate(hyphenatedInput).files[0];
    expect(() =>
