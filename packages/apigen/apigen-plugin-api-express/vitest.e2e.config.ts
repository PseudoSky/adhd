...
/**
 * Dedicated E2E lane for `apigen-plugin-api-express`.
 *
 * The resource-consuming suites — the ones that start real, live-dispatched
 * Express servers bound to real ports and drive them over real HTTP
 * round-trips — were extracted out of the default `test` target into sibling
 * `*.e2e.ts` files, with `*.spec.ts` stubs left at the original paths (see
 * each stub's header). This config runs ONLY those `*.e2e.ts` files, under the
