...
export function constructSeedTs(
  ...
  switch ($construct) {
    ...
    case 'number-special': {
      ...
      if (sentinel === '-Infinity') return -Infinity;
      throw new Error(
        `[constructSeedTs] unknown number-special sentinel: ${sentinel}`
      );
    }
    case 'int64':
    ...
    default:
      throw new Error(
        `[constructSeedTs] unsupported $construct logicalType: ${$construct}`
      );
  }
}
...
export function checkInvariantTs(
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    return `${pointer}: expected ${JSON.stringify(
      expected
  ...
  for (const v of vectors) {
    ...
    if (JSON.stringify(encoded) !== JSON.stringify(v.wire)) {
      results.push({
        ...
        pass: false,
        phase: 'encode',
        error: `encode mismatch: expected ${JSON.stringify(
          v.wire
        )}, got ${JSON.stringify(encoded)}`,
      });
      continue;
    }
    ...
    if (nc.mutate === 'wire' && v.logicalType === 'date-time') {
      ...
      negativeRed =
        typeof mutatedWire === 'string' && !mutatedWire.endsWith('Z');
    }
    ...
    results.push({
      ...
      error: negativeRed
        ? undefined
        : `negativeControl is vacuous: mutation "${
            nc.mutate
          }" to ${JSON.stringify(nc.to)} did not turn the check RED`,
    });
  }
...
export function runPythonMatrix(
  ...
  const vectorsFile = path.join(
    tmpDir,
    `apigen-gate-vectors-${process.pid}.json`
  );
  ...
  try {
    ...
    if (result.status !== 0) {
      ...
      throw new Error(
        `Python matrix subprocess failed (exit ${String(
          result.status
        )}):\n${detail}`
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Python matrix subprocess returned invalid JSON:\n${stdout}`
      );
    }
 ...
 */
export function getPythonHostManifest(
  workspaceRoot: string
): HostManifest | null {
  const pyHost = path.join(
    workspaceRoot,
    'packages',
    'apigen',
    'python',
...
export function discoverManifestHosts(workspaceRoot: string): HostManifest[] {
  ...
  for (const entry of fs.readdirSync(hostsDir, {
    recursive: true,
  }) as string[]) {
 ...
 */
export function runConformanceMatrix(
  workspaceRoot: string
): HostMatrixResult[] {
...
export function main(workspaceRootOverride?: string): void {
  ...
  for (const hostResult of matrixResults) {
    ...
    console.log(
      `       supportedIds: [${hostResult.manifest.supportedIds.join(', ')}]`
    );
    ...
    console.log(
      `       vectors: ${passes.length} passed, ${failures.length} failed`
    );
