...
export function makeOp(
  overrides: Partial<Operation> & Pick<Operation, 'id' | 'namespace' | 'path'>
): Operation {
  return {
    host: 'ts',
    kind: 'action',
...
export const ROUND_TRIP_OP: Operation = makeOp({
  ...
  path: [
    seg('humanize', ['humanize']),
    seg('humanizeBytes', ['humanize', 'bytes']),
  ],
  kind: 'action',
  async: true,
  streaming: false,
  ...
  typeText: {
    lang: 'ts',
    input: '{ bytes: number; precision?: number }',
    output: 'string',
  },
});
 ...
 */
export function assertOperationEqual(
  a: Operation,
  b: Operation
): null | string {
...
export const DESCRIPTOR_VECTORS = [
  {
    ...
    description:
      'A complete Operation round-trips through JSON serialization losslessly',
    input: ROUND_TRIP_OP,
  },
  {
...
export const NAMING_VECTORS = [
  ...
  },
  {
    ...
    description:
      'Two ids with identical path tokenization → CollisionDetectedError',
...
export const ENVELOPE_CASES = [
  {
    pluginId: 'auth',
    field: 'session',
    expectedKey: 'x-auth-session',
    expectedFlag: '--auth-session',
    expectedEnv: 'APIGEN_AUTH_SESSION',
  },
  {
    pluginId: 'adhd',
    field: 'trace-id',
    expectedKey: 'x-adhd-trace-id',
    expectedFlag: '--adhd-trace-id',
    expectedEnv: 'APIGEN_TRACE_ID',
  },
  {
    pluginId: 'rate',
    field: 'limit',
    expectedKey: 'x-rate-limit',
    expectedFlag: '--rate-limit',
    expectedEnv: 'APIGEN_RATE_LIMIT',
  },
] as const;
...
export function assertEnvelopeBinding(
  ...
  if (key !== expectedKey)
    return `envelopeKey: expected "${expectedKey}", got "${key}"`;
  if (flag !== expectedFlag)
    return `envelopeCliFlag: expected "${expectedFlag}", got "${flag}"`;
  if (env !== expectedEnv)
    return `envelopeEnvVar: expected "${expectedEnv}", got "${env}"`;
  // §9.1 rule 1: MCP _meta key === HTTP key
  if (meta !== key)
    return `envelopeMetaKey !== envelopeKey: "${meta}" !== "${key}"`;
  return null;
}
...
export const ERROR_MAPPING_VECTORS: Array<{
  ...
  {
    id: 'error.map.invalid_argument',
    code: 'invalid_argument',
    expected: { http: 400, grpc: 'INVALID_ARGUMENT', cli: 2, mcp: 'error' },
  },
  {
    id: 'error.map.unauthenticated',
    code: 'unauthenticated',
    expected: { http: 401, grpc: 'UNAUTHENTICATED', cli: 3, mcp: 'error' },
  },
  {
    id: 'error.map.permission_denied',
    code: 'permission_denied',
    expected: { http: 403, grpc: 'PERMISSION_DENIED', cli: 3, mcp: 'error' },
  },
  {
    id: 'error.map.not_found',
    code: 'not_found',
    expected: { http: 404, grpc: 'NOT_FOUND', cli: 4, mcp: 'error' },
  },
  {
    id: 'error.map.internal',
    code: 'internal',
    expected: { http: 500, grpc: 'INTERNAL', cli: 1, mcp: 'error' },
  },
];
 ...
 */
export function assertApiErrorCodeSurvivesSerialize(
  code: ApiErrorCode
): null | string {
...
export const VALIDATION_VECTORS: ValidationCase[] = [
  {
    ...
    description:
      'additionalProperties not restricted — extra keys pass schema but may cause dispatch errors',
    schema: {
      type: 'object',
    ...
    domainNote:
      'An unexpected field passes the schema but may be ignored or cause a typed dispatch error in strict hosts (Rust/Go).',
  },
  {
    id: 'validation.number-precision',
    description:
      'A JSON number within the schema range but beyond JS safe integer boundary',
    schema: {
      type: 'object',
  ...
  },
  {
    ...
    domainNote:
      'Standard AJV does not validate date-time values by default; Feb 30 passes schema validation but is a semantically invalid date.',
  },
  {
    id: 'validation.option-vs-missing',
    description:
      'An Option/nullable field being null passes where a domain function expects presence',
    schema: {
      type: 'object',
      properties: {
        user: {
    ...
    domainNote:
      'null satisfies the oneOf schema, but a domain function that destructures user.id will throw at runtime — validation passed, dispatch fails.',
  },
];
 ...
 */
export function assertLogicalTypeVectorWellFormed(
  v: LogicalTypeVector
): null | string {
  ...
  if (!validMutates.includes(v.negativeControl.mutate)) {
    return `${v.id}: negativeControl.mutate must be one of ${validMutates.join(
      ', '
    )}, got "${String(v.negativeControl.mutate)}"`;
  }
  // The negative control's mutation must differ from the canonical wire (otherwise vacuous).
  if (
    v.negativeControl.mutate === 'wire' &&
    JSON.stringify(v.negativeControl.to) === JSON.stringify(v.wire)
  ) {
 ...
 */
export function assertLogicalTypeVectorIdsUnique(
  vectors: LogicalTypeVector[]
): null | string {
 ...
 */
export function assertLogicalTypeVectorCoverage(
  vectors: LogicalTypeVector[]
): null | string {
...
export function assertWireMatchesFormat(v: LogicalTypeVector): null | string {
  ...
  if (typeof wire !== 'string') {
    ...
    return `${v.id}: expected wire to be a string for format "${String(
      fmt
    )}", got ${typeof wire}`;
  }
  switch (fmt) {
...
export function runAllVectors(): VectorResult[] {
  ...
  for (const c of ENVELOPE_CASES) {
    ...
    const err = assertEnvelopeBinding(
      c.pluginId,
      c.field,
      c.expectedKey,
      c.expectedFlag,
  ...
  for (const v of VALIDATION_VECTORS) {
    ...
    results.push({
      ...
      error: accepted
        ? undefined
        : `schema validator rejected a schema-valid value (should have accepted): ${JSON.stringify(
            v.schemaValidValue
          )}`,
    });
    ...
    results.push({
      ...
      error: rejectedInvalid
        ? undefined
        : `schema validator accepted "not-an-object" as valid for an object schema`,
    });
  }
  ...
  for (const v of logicalTypeVectors) {
    ...
    const mutatedV: LogicalTypeVector = {
      ...v,
      wire: v.negativeControl.to as Wire,
    };
    ...
    results.push({
      ...
      error: isRed
        ? undefined
        : `negativeControl.to "${JSON.stringify(
            v.negativeControl.to
          )}" still matches the canonical pattern — negative control is vacuous`,
    });
  }
