...
describe('C — Envelope binding', () => {
  ...
  it('[envelope.rule1] MCP _meta key equals HTTP header key (one mental model)', () => {
    ...
    expect(envelopeMetaKey('auth', 'session')).toBe(
      envelopeKey('auth', 'session')
    );
  });
...
describe('D — Error mapping', () => {
  ...
  it('[error.NEGATIVE] wrong HTTP status is detected', () => {
    ...
    const err = assertErrorMapping('not_found', {
      http: 999,
      grpc: 'NOT_FOUND',
      cli: 4,
      mcp: 'error',
  ...
  it('[error.NEGATIVE.grpc] wrong gRPC code is detected', () => {
    const err = assertErrorMapping('internal', {
      http: 500,
      grpc: 'WRONG',
      cli: 1,
      mcp: 'error',
...
describe('F — Logical type wire spec', () => {
  ...
  for (const v of logicalTypeVectors) {
    it(`[${
      v.id
    }.wire-format] wire matches canonical pattern for format "${String(
      v.schema['format'] ?? v.logicalType
    )}"`, () => {
  ...
  for (const v of logicalTypeVectors) {
    ...
    it(`[${v.id}.negative-control] negativeControl.to mutates wire to a non-conformant value`, () => {
      ...
      const mutatedV = {
        ...v,
        wire: v.negativeControl.to as ReturnType<(typeof v)['wire']['valueOf']>,
      };
      const err = assertWireMatchesFormat(
        mutatedV as Parameters<typeof assertWireMatchesFormat>[0]
      );
      ...
      expect(
        ...
        )}" still passes the format check — it is vacuous`
      ).not.toBeNull();
    });
  }
  ...
  it('[logical.number-special] NaN vector wire is "NaN"', () => {
    const v = logicalTypeVectors.find(
