...
// Run: npx vitest run src/test/plugin.e2e.ts --config vitest.e2e.config.ts -t "LIVE gRPC server"
      ...
      function runGoldenParityCheckInFreshProcess(): void {
        const result = spawnSync(
          ...
          [
            ...
            'run',
            '--config',
            path.join(repoRoot, 'packages/apigen/apigen-plugin-py-grpc/vitest.e2e.config.ts'),
            '-t',
