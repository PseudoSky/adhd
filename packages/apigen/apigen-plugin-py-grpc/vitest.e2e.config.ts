 ...
 * `python3` interpreter (a live gRPC server bound to an OS-assigned port,
 * driven by the external `grpcurl` binary), plus a `node`/`npx` subprocess for
 * the golden-parity negative control — was extracted out of the default `test`
 ...
 * only under the explicitly-invoked `nx run apigen-plugin-py-grpc:e2e`
...
export default mergeConfig(baseViteConfig as UserConfig, {
  test: {
    ...
    cache: {
      ...sharedCache,
      dir: '../../../node_modules/.vitest/apigen-plugin-py-grpc-e2e',
    },
    coverage: {
      ...sharedCoverage,
      provider: 'v8',
      reportsDirectory:
        '../../../coverage/packages/apigen/apigen-plugin-py-grpc-e2e',
    },
  },
} as UserConfig);
