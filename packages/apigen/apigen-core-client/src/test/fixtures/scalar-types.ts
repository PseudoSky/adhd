...
export async function echoReadonlyStringArray(
  xs: readonly string[]
): Promise<readonly string[]> {
  return xs;
}

export async function echoReadonlyNumberArray(
  xs: readonly number[]
): Promise<readonly number[]> {
  return xs;
}

export async function echoReadonlyArrayGeneric(
  xs: ReadonlyArray<string>
): Promise<ReadonlyArray<string>> {
  return xs;
}

export async function echoNestedReadonlyArray(
  xs: readonly string[][]
): Promise<readonly string[][]> {
  return xs;
}
...
export async function nestedDate(
  d: Date
): Promise<{ at: Date; label: string }> {
  return { at: d, label: 'x' };
}
...
export async function nestedDateAndArray(
  d: Date
): Promise<{ at: Date; dates: Date[] }> {
  return { at: d, dates: [d] };
}
...
export async function nestedBigint(
  n: bigint
): Promise<{ n: bigint; label: string }> {
  return { n, label: 'x' };
}

/** Nested Uint8Array in object: output.properties.data must be {type:string,format:byte} */
export async function nestedUint8Array(
  data: Uint8Array
...
export async function nestedBuffer(
  data: Buffer
): Promise<{ data: Buffer; len: number }> {
  return { data, len: data.length };
}
...
export async function mapDateValue(
  m: Map<string, Date>
...
export async function echoReadonlyMap(
  m: ReadonlyMap<number, string>
): Promise<ReadonlyMap<number, string>> {
  return m;
}

export async function echoReadonlySet(
