...
export async function withDefaultImport(
  p: Decimal
): Promise<{ cost: Decimal }> {
  return { cost: p };
}

/** Decimal[] (default import) nested in object → amounts.items must be {type:string,format:decimal} */
export async function withDecimalArray(
  p: Decimal
): Promise<{ amounts: Decimal[] }> {
  return { amounts: [p] };
}
...
export async function plainObject(
  x: number
): Promise<{ a: number; b: string }> {
