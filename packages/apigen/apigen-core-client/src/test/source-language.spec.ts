...
describe('pluginConsumesSource', () => {
  ...
  describe('positive matches', () => {
    it('ts plugin consumes .ts', () =>
      expect(pluginConsumesSource(tsPlugin, 'api.ts')).toBe(true));
    it('ts plugin consumes .tsx', () =>
      expect(pluginConsumesSource(tsPlugin, 'api.tsx')).toBe(true));
    it('ts plugin consumes .mts', () =>
      expect(pluginConsumesSource(tsPlugin, 'api.mts')).toBe(true));
    it('ts plugin consumes .cts', () =>
      expect(pluginConsumesSource(tsPlugin, 'api.cts')).toBe(true));
    it('py plugin consumes .py', () =>
      expect(pluginConsumesSource(pyPlugin, 'service.py')).toBe(true));
  });

  describe('negative matches — language mismatch', () => {
    it('ts plugin rejects .py', () =>
      expect(pluginConsumesSource(tsPlugin, 'service.py')).toBe(false));
    it('py plugin rejects .ts', () =>
      expect(pluginConsumesSource(pyPlugin, 'api.ts')).toBe(false));
    it('py plugin rejects .tsx', () =>
      expect(pluginConsumesSource(pyPlugin, 'api.tsx')).toBe(false));
  });

  describe('negative matches — unknown extension', () => {
    it('never routes README.md', () =>
      expect(pluginConsumesSource(tsPlugin, 'README.md')).toBe(false));
    it('never routes schema.json', () =>
      expect(pluginConsumesSource(pyPlugin, 'schema.json')).toBe(false));
    it('never routes Makefile', () =>
      expect(pluginConsumesSource(tsPlugin, 'Makefile')).toBe(false));
  });
...
describe('sourcesForPlugin', () => {
  ...
  it('returns only .go files for a go plugin', () => {
    expect(sourcesForPlugin({ language: 'go' }, mixed)).toEqual([
      'cmd/main.go',
    ]);
  });
  ...
  it('never includes files with unrecognised extensions', () => {
    expect(
      sourcesForPlugin({ language: 'ts' }, ['README.md', 'schema.json'])
    ).toEqual([]);
  });
  ...
  it('preserves insertion order of matched files', () => {
    ...
    expect(sourcesForPlugin({ language: 'ts' }, files)).toEqual([
      'z.ts',
      'a.ts',
      'm.ts',
    ]);
