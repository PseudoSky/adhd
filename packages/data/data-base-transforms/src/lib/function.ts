 ...
 */
export type CallbackFunctionTyped<RT, Params extends []> = (
  ...args: Params
) => RT;
...
export function toPath(path: string | string[]): string[] {
  return typeof path === 'string'
    ? path.split(STR_PATH_REGEX).filter(Boolean)
    : path;
}
...
export function throttle<T extends CallbackFunctionVariadic>(
  func: T,
  timeFrame: number
) {
  let lastTime = 0;
...
export function partial<F extends CallbackFunctionVariadic>(
  func: CallbackFunctionVariadic,
  ...boundArgs: Parameters<F>
) {
  return (...remainingArgs: Parameters<F>) =>
