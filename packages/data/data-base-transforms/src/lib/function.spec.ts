...
describe('functional', () => {
  it('setters', () => {
    ...
    expect(functional.set({}, 'a.b.c', 1)).toMatchObject({
      a: { b: { c: 1 } },
    });
    expect(functional.set({}, 'a[0].c', 1)).toMatchObject({ a: [{ c: 1 }] });
    expect(functional.set({}, 'a.b.c', [])).toMatchObject({
      a: { b: { c: [] } },
    });
  ...
  it('set', () => {
    const testObject = { a: [{ b: { c: 3 } }] };
    const testObject2 = { a: [{ b: { c: 3, e: 1 } }] };
    expect(functional.set(testObject, 'a[0].b.e', 1)).toMatchObject(
      testObject2
    );
  });
  it('getAll', () => {
