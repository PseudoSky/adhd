...
describe('collections', () => {
  ...
  describe('sortByKey', () => {
    it('should sort an array of objects by a key', () => {
      ...
      expect(data.sort(sortByKey('id')).map((item) => item.id)).toEqual([
        1, 2, 3,
      ]);
    });
  });
...
describe('collections.ts', () => {
  it('should work', () => {
    ...
    const CollectionEx = ArrEx.map((i, index) => ({
      ...ObjSample,
      sort: i,
      value: i * index,
      nested: ObjSample,
    ...
    expect(collection.sort(sortByKey(prop)).map((e) => e[prop])).toEqual([
      0, 1, 2, 3, 4, 5,
    ]);
    expect(
      sortBy(
    ...
    expect(
      overEach([boolCheckExists, boolCheckNotExists, boolCheckNotEqOne])(arr)
    ...
    expect(pickBy([orig, orig], boolCheckExists)).toEqual([
      { key: 'value' },
      { key: 'value' },
    ]);
    expect(keySelect(key)({ key: 9 })).toEqual(9);
    expect(pluck(CollectionEx, key)).toEqual([
      'value',
      'value',
      'value',
      'value',
    ...
    expect(maxByProp(collection, prop)).toEqual({
      key: 'value',
      nested: { key: 'value' },
      sort: 5,
      value: 20,
    });
    expect(minByProp(collection, prop)).toEqual({
      key: 'value',
      nested: { key: 'value' },
      sort: 0,
      value: 0,
    ...
    expect(rangeByProp(collection, prop)).toEqual({
      key: prop,
      min: 0,
      max: 5,
    });
    ...
    expect(maxBy(collection, (o) => o.value)).toEqual({
      key: 'value',
      nested: { key: 'value' },
      sort: 5,
      value: 20,
    });
    expect(minBy(collection, (o) => o.value)).toEqual({
      key: 'value',
      nested: { key: 'value' },
      sort: 0,
      value: 0,
  ...
  describe('isMatch', () => {
    ...
    it('should handle mixed nested structures', () => {
      expect(isMatch({ a: [1, { b: 2 }], c: 3 }, { a: [1, { b: 2 }] })).toEqual(
        true
      );
    });
  });
