...
describe('object', () => {
  it('should work', () => {
    ...
    expect(objectUtils.objectDifference({ a: 1, b: 2 }, {})).toEqual({
      a: 1,
      b: 2,
    });
    expect(objectUtils.entries({ a: 1, b: 2 })).toEqual([
      ['a', 1],
      ['b', 2],
    ]);
    expect(
    ...
    expect(objectUtils.unZipObject({ a: 1, b: 2 })).toEqual([
      ['a', 1],
      ['b', 2],
    ]);
    expect(objectUtils.values({ a: 1, b: 2 })).toEqual([1, 2]);
    expect(
      objectUtils.zipObject([
        ['a', 1],
        ['b', 2],
      ])
    ).toEqual({ a: 1, b: 2 });
