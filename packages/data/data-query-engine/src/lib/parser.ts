...
export function compileWhere(
  ...
  const filters = _.allPaths(
    query,
    (key) =>
      key.toString().startsWith('_') &&
      !['_and', '_or', '_not'].includes(key.toString())
  );
  const fields = _.allPaths(
    query,
    (key, _, o) =>
      !/^[0-9]+$/.test(`${key}`) &&
      !key.toString().startsWith('_') &&
      Object.keys(o[key] as object).every(
        (e) => e.startsWith('_') && !['_and', '_or', '_not'].includes(e)
      )
  );
  ...
  const compiledFilters: CompiledFilter[] = filters.reduce(
    (res: CompiledFilter[], item: string[]) => {
      const key = item.join('.');
      const depKey = item.slice(0, -1).join('.');
      const fieldKey = item
        .filter((e) => !e.startsWith('_') && !/^[0-9]+$/.test(e))
        .join('.');
  ...
  deps = fields.reduce((res, item) => {
    ...
    if (/_or$/.test(parent)) {
      const implicitKey =
        item.slice(0, parent.split('.').length + 1).join('.') + '._and';
  ...
  const allDeps = logical.reduce((res, item) => {
    ...
    return {
      ...res,
      [parent]: (parent in res ? res[parent] : []).concat([item.join('.')]),
    };
  }, deps as Record<string, string[]>);

  const sortedLogical = [...logical].sort((a, b) => b.length - a.length);
  const nonLogicalFilters = fields.filter(
    (e) => !e.some((pk) => ['_and', '_or', '_not'].includes(pk))
  );
  ...
  return (obj: Record<string, unknown>): boolean => {
    ...
    const finalRoot = hasRootDeps
      ? allDeps['<root>'].every((e) => results[e])
      : true;
...
export function parseOrderBy(
  query: string | OrderByExpression | OrderByExpression[]
): OrderByOperation[] {
  ...
  if (_.isString(query)) {
    ...
    const entries = _.allPaths(query).sort(([k1], [k2]) =>
      _.defaultSort(k1, k2)
    );
