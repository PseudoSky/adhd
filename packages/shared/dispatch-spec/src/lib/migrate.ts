 ...
 */
interface MigrationStep { from: number; to: number; apply: (dag: Record<string, unknown>) => void }

const MIGRATIONS: MigrationStep[] = [
  { from: 2, to: 3, apply: (dag) => {
    if (!dag["milestones"]&&dag["nodes"]) { dag["milestones"]=dag["nodes"]; delete dag["nodes"]; }
    const ops = dag["operations"]; if (ops&&typeof ops==="object"&&!Array.isArray(ops)) dag["operations"]=Object.values(ops as Record<string,unknown>);
  }},
  { from: 3, to: 4, apply: (dag) => {
    if (!dag["providers"]||Object.keys(dag["providers"]as object).length===0) dag["providers"]={
...
export function migrateDag(fromVersion: number, toVersion: number, dag: Record<string, unknown>): void {
  if (fromVersion===toVersion) return;
  let current=fromVersion;
  for (const step of [...MIGRATIONS].sort((a,b)=>a.from-b.from)) {
    if (step.from===current) { step.apply(dag); current=step.to; if (current===toVersion) { dag["schema_version"]=toVersion; return; }}
  }
  throw new Error(`migrateDag: no path from v${fromVersion} to v${toVersion}. Available: [${MIGRATIONS.map(m=>`v${m.from}→v${m.to}`).join(", ")}]`);
}
