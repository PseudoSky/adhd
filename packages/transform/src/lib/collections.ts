...
export const keyByArray = (array, key) => (array || []).reduce((r, x) => ({ ...r, [key ? x[key] : x]: x }), {});
...
export function isMatchType(obj, target){
...
export function overSome(checks){
  return item => checks.some(check => check(item))
}

export function overEvery(checks){
  return item => checks.every(check => check(item))
}

export function overEach(arr){
  return (...args) => arr.map(func => func(...args))
}

export function omitBy(orig, check) {
  const obj = { ...orig }
...
export const sortByKey = (key) => { return extractThen(key, defaultSort) };

export const sortBy = (arr, prop?: string, cmp=defaultSort) => !prop && !!cmp? arr.sort(cmp) : sortByProp(arr, prop, cmp);
...
export function minByProp(arr, prop){
  return sortByProp(arr, prop)[0]
}

export function filterExclude(arr, obj={}){
  return arr.filter(e => !isMatch(e, obj))
}

export function filterInclude(arr, obj={}){
  return arr.filter(e => isMatch(e, obj))
}
...
export function uniqueBy(arr, props){
...
export function rangeByProp(arr, prop){
...
export function rangeByProps(arr, props){
