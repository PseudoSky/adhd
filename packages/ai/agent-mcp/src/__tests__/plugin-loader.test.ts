...
describe('findConfigFile', () => {
  it('returns null when explicit path points to a nonexistent file', () => {
    expect(
      findConfigFile('/tmp/definitely-does-not-exist-agent-mcp.json')
    ).toBeNull();
  });
...
describe('loadConfigFile', () => {
  it('returns empty plugins when no config file is found', () => {
    ...
    expect(
      loadConfigFile('/tmp/__nonexistent_agent_mcp_test__.json').plugins
    ).toEqual([]);
  });
  ...
  it('installs a plugin declared in the config file (absolute path)', async () => {
    ...
    try {
      const pluginPath = writePlugin(
        dir,
        'plugin.mjs',
        markerPlugin(markerPath)
      );
      const configPath = join(dir, 'config.json');
      writeFileSync(
        configPath,
        JSON.stringify({ plugins: [{ module: pluginPath }] })
      );
  ...
  it('passes the config block to the plugin factory as ctx.config', async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          plugins: [
            { module: pluginPath, config: { threshold: 42, label: 'test' } },
          ],
        })
      );
...
describe('loadExternalPlugins — failure resilience', () => {
  it('skips a plugin whose factory throws and continues — teeth: marker absent', async () => {
    ...
    try {
      ...
      await expect(
        loadExternalPlugins(new HookRegistry(), {}, { configPath })
  ...
  it('loads budget plugin via config file and enforces maxModelCalls:1 on second model call', async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          plugins: [
            { module: BUDGET_PLUGIN_DIST, config: { maxModelCalls: 1 } },
          ],
        })
      );
