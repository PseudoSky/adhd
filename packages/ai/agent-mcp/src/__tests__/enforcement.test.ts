...
import type { IEnforcementError, IToolWarning } from '@adhd/agent-base-types';
...
describe('HookRegistry.enforce()', () => {
  it('runs enforcement handlers and resolves when none throw', async () => {
    ...
    await expect(
      registry.enforce('pre:model_request', {
  ...
  it('propagates throws from enforcement handlers (no swallow)', async () => {
    ...
    registry.registerEnforcement('pre:model_request', () => {
      throw err;
    });
  ...
  it('runs enforcement AFTER observational emit() on the same event', async () => {
    ...
    registry.register('pre:model_request', () => {
      order.push('observe');
    });
    registry.registerEnforcement('pre:model_request', () => {
      order.push('enforce');
    ...
    const ctx = makeExecCtx();
    await registry.emit('pre:model_request', {
      executionContext: ctx,
      messages: [],
      tools: [],
    });
    await registry.enforce('pre:model_request', {
      executionContext: ctx,
      messages: [],
      tools: [],
    });
  ...
  it('observational emit() is unaffected when enforcement would throw', async () => {
    ...
    await expect(
      registry.emit('pre:model_request', {
        executionContext: ctx,
        messages: [],
        tools: [],
      })
...
describe('Orchestrator — BUDGET_EXCEEDED via BudgetPlugin', () => {
  it('task fails with BUDGET_EXCEEDED when enforcement blocks second model call', async () => {
    const { buildHarness, drainQueue, createSessionAndAgent } = await import(
      './integration/harness.js'
    );
    const { ScriptedProvider } = await import(
      './integration/scripted-provider.js'
    );
    ...
    const provider = new ScriptedProvider([
      {
        type: 'tool_calls',
        toolCalls: [{ server: 'noop', tool: 'noop_tool', arguments: {} }],
      },
    ...
    try {
      ...
      harness.hooks.register('post:model_response', () => {
        modelCallCount++;
      });
      harness.hooks.registerEnforcement('pre:model_request', () => {
        if (modelCallCount >= 1) {
          const err: IEnforcementError = {
            ...
            code: 'BUDGET_EXCEEDED',
            message:
              'BUDGET_EXCEEDED: maxModelCalls limit is 1, current value is 1',
          };
          throw err;
        }
      });
        ...
        orchestrator: {
          run: (input: Parameters<typeof harness.orchestrator.run>[0]) =>
            harness.orchestrator.run({
              ...input,
              provider,
              registry: stubRegistry as Parameters<
                typeof harness.orchestrator.run
              >[0]['registry'],
            }),
        } as Orchestrator,
      };
      ...
      const allTasks = (harness.db as any)
        .select()
        .from(tasksTable)
      ...
      await taskTool(
        {
          ...
          prompt: 'should complete because no enforcement',
          background: true,
        },
        patchedDeps
      );
      ...
      harness.hooks.registerEnforcement('pre:tool_call', () => {
        ...
        if (toolCallCount >= 1) {
          const err: IEnforcementError = {
            ...
            code: 'BUDGET_EXCEEDED',
            message:
              'BUDGET_EXCEEDED: tool maxCalls limit is 0, current value is 0',
          };
          throw err;
        }
      });
  ...
  it('warning mode: task continues and tool result contains warning', async () => {
    ...
    try {
      ...
      await taskTool(
        {
          session_id: sessionId,
          prompt: 'call tools and handle warning',
          background: true,
        },
        patchedDeps
      );
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const messages = (harness.db as any)
        .select()
        .from(messagesTable)
      ...
      const warningMsg = toolMessages.find((m: { toolResults?: string }) => {
        ...
        try {
          ...
          return (
            ...
            parsed.some((r: { result?: { text?: string } }) =>
