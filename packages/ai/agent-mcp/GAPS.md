...
- **Critical FK-cascade fix for that recreate:** `task_events.task_id → tasks.id ON DELETE CASCADE` means the migration's `DROP TABLE tasks` would cascade-delete **every existing `task_events` row** if FK enforcement is on. drizzle-kit emits `PRAGMA foreign_keys=OFF` at the top of the migration, but **that statement is a silent no-op** — SQLite ignores `PRAGMA foreign_keys` inside a transaction, and the migrator wraps each migration in one. So the migration must be driven by `runMigrationsOn` (`db/migrate-runner.ts`), which toggles FK enforcement off on the *connection* (where no transaction is open) for the duration of the run and restores it after. Verified on a copy of the real production DB: 103 → 103 `task_events` rows survive the upgrade (without the runner: 103 → 0). Regression test: `migration-fk-cascade.integration.test.ts` (T9), teeth-checked.
