...
describe('validateDagJson', () => {
  ...
  describe('dispatch_log[].provider (DEBT-DISPATCH-019)', () => {
    function dagWithProvider(provider: unknown): Record<string, unknown> {
      return miniDag({
        dispatch_log: [
          {
            id: 'd-1',
    ...
    it('rejects an unrecognized provider value', () => {
      const r = validateDagJson(dagWithProvider('not-a-real-provider'));
      expect(r.valid).toBe(false);
      expect(
        r.errors.some((e) => e.path === 'dispatch_log[d-1].provider')
      ).toBe(true);
    });

    it('accepts provider: teammate (real production value)', () => {
      const r = validateDagJson(dagWithProvider('teammate'));
      expect(r.valid).toBe(true);
    });

    it('accepts provider: claudecli (real production value)', () => {
      const r = validateDagJson(dagWithProvider('claudecli'));
      expect(r.valid).toBe(true);
    });

    it('accepts every documented provider value', () => {
      for (const provider of [
        ...
        const r = validateDagJson(dagWithProvider(provider));
        expect(r.valid, `provider=${provider} should be valid`).toBe(true);
      }
    });

    it('rejects a missing provider field', () => {
      const d = dagWithProvider('anthropic');
      const entry = (d['dispatch_log'] as Record<string, unknown>[])[0];
