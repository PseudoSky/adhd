...
describe('DagClient', () => {
  ...
  describe('load', () => {
    ...
    it('throws when no dag exists', async () => {
      await expect(client.load()).rejects.toThrow(
        'No dag found — call saveDag first'
      );
    });
  });
  ...
  describe('updateOperationStatus', () => {
    ...
    it('throws for unknown operation', async () => {
      serializer.setDag(makeFixtureDag());
      await expect(
        client.updateOperationStatus('nope', 'complete')
      ).rejects.toThrow('Operation nope not found');
    });
  });
  ...
  describe('clearPending', () => {
    ...
    it('throws for unknown milestone', async () => {
      serializer.setDag(makeFixtureDag());
      await expect(client.clearPending('nope')).rejects.toThrow(
