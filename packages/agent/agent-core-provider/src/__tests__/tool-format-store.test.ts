...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'agent-provider-tool-format-test-')
  );
  ...
  it('round-trips an Anthropic bash (unsupported) row with note set and typeTag null', () => {
    ...
    // ── REOPEN + READ ──────────────────────
    {
      ...
      expect(row.note).toBe(
        'Anthropic bash requires a local execution loop; not supported'
      );
...
describe('ToolFormatStore — CRUD', () => {
  ...
  it('lists all tool format rows for a given provider', () => {
    ...
    store.create({
      ...
      canonicalTool: 'bash',
      emitShape: 'unsupported',
      note: 'needs local loop',
    });
    store.create({
      providerId: 'anthropic',
      canonicalTool: 'my_custom',
      emitShape: 'custom',
    });
    ...
    expect(anthropicFormats.map((f) => f.canonicalTool).sort()).toEqual([
      'bash',
      'my_custom',
      'web_search',
    ]);
