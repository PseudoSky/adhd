...
function makeLookup(rows: Record<string, ToolFormat>): ToolFormatLookup {
...
describe('emitTool — server-side tool (FEAT-007 server-side branch)', () => {
  ...
  it('emitted server-side entry has no description field (mutually exclusive with custom)', () => {
    const emitted = emitTool(
      webSearchTool,
      'anthropic',
      anthropicLookup
    ) as EmittedServerSideTool;
