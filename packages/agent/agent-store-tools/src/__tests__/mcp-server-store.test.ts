...
function expectMcpServerStoreError(
  fn: () => unknown,
  code: McpServerStoreErrorCode
): void {
  try {
    fn();
    expect.fail(
      `Expected a McpServerStoreError with code '${code}' to be thrown`
    );
  } catch (err: unknown) {
...
describe('McpServerStore — persistence (close + reopen proves disk write)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'agent-tool-registry-mcp-persist-')
    );
    dbPath = path.join(tmpDir, 'registry.db');
  });
  ...
  it('[inv:reopen-proves-persistence] server round-trips through a close+reopen — JSON columns survive', () => {
    ...
    expect(readBack.providedToolIds).toEqual([
      'file_read',
      'file_write',
      'file_list',
    ]);
    ...
    expect(
      (readBack.configSchema as typeof FILESYSTEM_CONFIG_SCHEMA).type
    ).toBe('object');
    expect(
      (readBack.configSchema as typeof FILESYSTEM_CONFIG_SCHEMA).required
    ).toEqual(['roots']);
    expect(
      (readBack.configSchema as typeof FILESYSTEM_CONFIG_SCHEMA).properties
        .roots.type
    ).toBe('array');
