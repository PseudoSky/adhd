...
function expectToolStoreError(
  fn: () => unknown,
  code: ToolStoreErrorCode
): void {
  try {
    fn();
...
const COMPUTE_TYPE = {
  slug: 'compute',
  description: 'Computation and processing',
};
...
describe('ToolStore — tool_types (lookup table, never enum)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'agent-tool-registry-test-')
    );
...
describe('ToolStore — tools table', () => {
  ...
  it('throws TOOL_NOT_FOUND when reading a missing tool', () => {
    expectToolStoreError(() => store.read('nonexistent'), 'TOOL_NOT_FOUND');
  });
...
describe('ToolStore — persistence (close + reopen proves disk write)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'agent-tool-registry-persist-')
    );
    dbPath = path.join(tmpDir, 'registry.db');
  });
