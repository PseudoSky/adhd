...
function expectAgentToolStoreError(
  fn: () => unknown,
  code: AgentToolStoreErrorCode
): void {
  try {
    fn();
    expect.fail(
      `Expected an AgentToolStoreError with code '${code}' to be thrown`
    );
  } catch (err: unknown) {
...
describe('AgentToolStore — grant / listForAgent / revoke', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'agent-tool-registry-junction-')
    );
  ...
  it('all three permission levels are accepted and stored verbatim', () => {
    ...
    const byTool = Object.fromEntries(
      grants.map((g) => [g.toolName, g.permission])
    );
...
describe('AgentToolStore — persistence (close + reopen proves disk write)', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'agent-tool-registry-junction-persist-')
    );
    dbPath = path.join(tmpDir, 'registry.db');
  });
...
describe('AgentToolStore — [inv:no-cross-pkg-fk] agent_slug is a logical reference', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'agent-tool-registry-junction-nofk-')
    );
    dbPath = path.join(tmpDir, 'registry.db');
  });
  ...
  it('agent_slug with no matching agents-table row is accepted (no FK constraint)', () => {
    ...
    const grants = store.listForAgent(
