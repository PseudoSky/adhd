...
describe('seed-and-roundtrip [seed-and-roundtrip.1]', () => {
  ...
  it('[inv:reopen-proves-persistence] binding round-trips after close+reopen', () => {
    ...
    const shellExecOnClaudeCode = bindingStore.resolve(
      'shell_exec',
      'claude_code'
    );
    const shellExecOnClaudeApi = bindingStore.resolve(
      'shell_exec',
      'claude_api'
    );
    const fileReadOnClaudeCode = bindingStore.resolve(
      'file_read',
      'claude_code'
    );
    const webFetchOnClaudeCode = bindingStore.resolve(
      'web_fetch',
      'claude_code'
    );
