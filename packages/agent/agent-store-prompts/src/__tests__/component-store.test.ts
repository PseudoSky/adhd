...
describe('ComponentStore', () => {
  ...
  beforeAll(() => {
    ...
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'agent-registry-test-')
    );
  ...
  describe('prompt_types', () => {
    ...
    it('throws COMPONENT_TYPE_NOT_FOUND for unknown type', () => {
      expect(() => store.readType('nonexistent-type')).toThrowError(
        ComponentError
      );
      expect(() => store.readType('nonexistent-type')).toThrowError(
        'not found'
  ...
  describe('version()', () => {
    ...
    it('readVersion() on missing (slug, version) pair throws COMPONENT_VERSION_NOT_FOUND', () => {
      store.create({
        slug: 'only-v1',
        type: 'rule',
        content: 'just one version',
      });
      expect(() => store.readVersion('only-v1', 99)).toThrowError(
        ComponentError
      );
    });
  ...
  describe('list()', () => {
    ...
    it('combined type + shared filter', () => {
      ...
      expect(
        result.every((c) => c.type === 'rule' && c.isShared === true)
      ).toBe(true);
    });
  });
  ...
  describe('versionId surrogate (Decision 5)', () => {
    ...
    it('resolveVersionId throws COMPONENT_VERSION_NOT_FOUND for a missing version', () => {
      ...
      expect(() => store.resolveVersionId('vid-missing', 99)).toThrowError(
        ComponentError
      );
    });
  });
  ...
  describe('persistence across DB reopen', () => {
    it('component persists after closing and reopening the DB handle', () => {
      ...
      store.create({
        slug: 'persist-test',
        type: 'rule',
        content: 'I must survive a reopen',
      });
