...
const stubConfig: EngineConfig = {
    server: { contextLimit: 0, defaultMaxTokens: 8192 },
    queue: { concurrency: 5 },
    sse: { baseUrl: "http://localhost" },
    plugins: { entries: [] },
...
describe("tool-naming", () => {
    ...
    describe("registry resolveToolName round-trip", () => {
        it("resolves a model-normalized name back to the real {server,tool}", async () => {
...
describe("OpenAIProvider resolves a bare tool call (DEBT-004 consumer)", () => {
    function providerReturning(toolName: string): OpenAIProvider {
        const p = new OpenAIProvider(
            { type: "openai", model: "test", baseURL: "http://localhost:1234/v1" } as never,
            stubConfig
        );
