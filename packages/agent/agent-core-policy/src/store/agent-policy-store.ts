...
export class AgentPolicyStore {
  ...
  attach(input: AgentPolicyAttachInput): AgentPolicyRow {
    ...
    const row = {
      ...
      overrideConfig: (input.overrideConfig ?? null) as unknown as
        | string
        | null,
  ...
  listForAgent(agentSlug: string): AgentPolicyRow[] {
    ...
    return rows.map((r: typeof agentPoliciesTable.$inferSelect) =>
      this._toRow(r)
    );
  }
  ...
  private _toRow(row: typeof agentPoliciesTable.$inferSelect): AgentPolicyRow {
    return {
      agentSlug: row.agentSlug,
      policySlug: row.policySlug,
      // drizzle returns parsed JS value for `mode:"json"` columns.
      overrideConfig:
        (row.overrideConfig as Record<string, unknown> | null) ?? null,
      isMandatory: Boolean(row.isMandatory),
