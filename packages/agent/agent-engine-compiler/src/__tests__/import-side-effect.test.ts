 ...
 * boot, lazily via `await import(...)`) silently materialized
 * `<cwd>/data/registry.db` on disk.
 *
...
describe('import-time side effect — @adhd/agent-engine-compiler', () => {
  ...
  it('importing the package barrel does not create ./data/registry.db (or ./data/ at all) in the invoking cwd', async () => {
    scratchCwd = mkdtempSync(join(tmpdir(), 'agent-engine-compiler-import-'));
