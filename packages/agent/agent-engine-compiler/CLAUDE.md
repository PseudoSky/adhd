...
**[`../agent-generator-plugin/REGISTRY-PACKAGE-RULES.md`](../agent-generator-plugin/REGISTRY-PACKAGE-RULES.md)** —
read it before changing this package. (Fixed `BUG-AGENTMCP-DOCLINK-001` — this
previously linked to a nonexistent `../agent-nx/` directory.) The
non-negotiables, in brief:

- **`platform:node`.** Never import browser code (`react`, `window`, `document`,
  CSS). Pure Node + SQLite.
- **One shared SQLite file, opened via DI — never a module-scope singleton.**
  Every registry-family package opens its own Drizzle instance against the
  SAME file, resolved via `@adhd/agent-core-env`'s `resolveRegistryDbPath()` /
  `openRegistryDb()` (lazy — never opens at import time; importing this
  package must never create a file as a side effect — see
  `docs/environment/agent-base-env/DESIGN.md`). All of this package's tables
  are prefixed **`compiler_`**. NO `ATTACH DATABASE`. NO cross-package SQL
  foreign keys — cross-package keys are logical, resolved at compile time.
  Within-package FKs ARE real (`.references()`).
