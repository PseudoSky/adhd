...
export const useFileDownload = ({
  ...
  const worker = useMemo(() => {
    if (
      ...
      new Blob([JSON.stringify(data)]).size > MAX_FILE_SIZE
    ) {
  ...
  const downloadFile = useCallback(
    async (fileType: FileType = 'json') => {
      ...
      try {
        ...
        const cacheKey = `${JSON.stringify(data)}_${fileType}_${JSON.stringify(
          options
        )}`;
        let blob: Blob;
        ...
        if (options.compression) {
          ...
          try {
            ...
            if (compressedBlob.size < blob.size) {
              ...
              console.warn(
                'Compression resulted in larger file, using original'
              );
            }
