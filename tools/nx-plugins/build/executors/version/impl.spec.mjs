...
function makeSpawnSyncMock(state) {
  return (cmd, args = [], _opts = {}) => {
    ...
    if (cmd === 'npm' && args[0] === 'view' && args[2] === 'versions') {
      return {
        status: 0,
        stdout: JSON.stringify(state.publishedVersions ?? []),
        stderr: '',
      };
    }
    if (cmd === 'npm' && args[0] === 'view' && args[2] === 'dist.integrity') {
      if (state.publishedIntegrity == null)
    ...
    if (
      ...
      writeFileSync(
        join(workDir, 'local-fake.tgz'),
        state.localTgzContent ?? 'local-tgz-bytes\n'
      );
      return {
        status: 0,
        stdout: JSON.stringify([{ filename: 'local-fake.tgz' }]),
        stderr: '',
      };
    }
    if (cmd === 'npm' && args[0] === 'pack') {
      ...
      return {
        status: 0,
        stdout: JSON.stringify([{ filename: 'fake-0.0.0.tgz' }]),
        stderr: '',
      };
    }
    if (cmd === 'tar') {
    ...
    if (
      ...
      if (state.changelogWrite) {
        for (const [abs, contents] of Object.entries(state.changelogWrite))
          writeFileSync(abs, contents);
      }
      return {
        status: state.changelogStatus ?? 0,
        stdout: '',
        stderr: state.changelogStatus ? 'boom' : '',
      };
    }
    throw new Error(
      `unexpected spawnSync in test mock: ${cmd} ${JSON.stringify(args)}`
    );
  };
}
...
test('DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): a REAL bump never invokes the deps-plugin sync/check executors — only reconcileInternalRangesFromDisk runs', async (t) => {
  ...
  try {
    makeFiles(join(rootDir, 'packages/pkg-a'), {
      'package.json': JSON.stringify(
        { name: '@adhd/pkg-a', version: '2.0.0' },
        null,
        2
      ),
    });
    ...
    const { pkgRoot, context } = makeProject({
      ...
      srcPkg: localPkg,
      distFiles: {
        'package.json': JSON.stringify(localPkg),
        'index.js': 'export const x = 2;\n',
      }, // NEW code -> real bump
    });
    context.projectsConfigurations.projects['pkg-a'] = {
      root: 'packages/pkg-a',
    };
    ...
    assert.equal(
      state.eslintCalls.length,
      0,
      'a real bump must NEVER invoke the deps-plugin sync/check executors — the upstream lint->sync-deps pass already covered this project'
    );
    ...
    assert.equal(
      after.dependencies['@adhd/pkg-a'],
      '^2.0.0',
      'the internal range must still be reconciled — by reconcileInternalRangesFromDisk alone'
    );
  } finally {
...
test('DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): a dry run never invokes the deps-plugin sync/check executors — only reconcileInternalRangesFromDisk runs (read-only)', async (t) => {
  ...
  try {
    ...
    assert.equal(
      state.eslintCalls.length,
      0,
      'a dry run must NEVER invoke the deps-plugin sync/check executors either'
    );
...
test('cache HIT, unchanged: ZERO network calls (no npm/tar spawnSync at all), no bump', async (t) => {
  ...
  try {
    ...
    writePublishedState(rootDir, {
      '@adhd/pkg-b': {
        version: '1.0.0',
        normalizedHash: normalizedHash(localDist),
        publishedIntegrity: 'sha512-whatever',
      },
    });
    ...
    assert.deepEqual(
      networkCalls(state),
      [],
      'a cache hit must NEVER touch npm or tar'
    );
    assert.equal(
      state.eslintCalls.length,
      0,
      'DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): version must NEVER invoke the deps-plugin sync/check executors — range reconciliation is on-disk only'
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    );
    assert.equal(
      after.version,
      '1.0.0',
      'unchanged vs the cached published hash -> no bump'
    );
    ...
    assert.equal(
      syncDepsRecords.length,
      0,
      'no nested sync-deps metrics record — the in-process call that produced it is gone'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('BUG-BUILD-ASSETS-CACHE-STALE-AFTER-CLEAN-001: a dist/ MISSING README.md (as a stale `assets` cache-hit after a fresh `build` clean would leave it) is repaired before hashing, so an unchanged package does NOT spuriously bump', async (t) => {
  ...
  try {
    ...
    } = require('../manifest/generate-manifest.js');
    const correctDistManifest = generateDistManifest(srcPkg, {});
    const distFilesCorrect = {
      'package.json': JSON.stringify(correctDistManifest),
      'index.js': 'export const x = 1;\n',
      'README.md': README,
    };
    ...
    assert.equal(
      existsSync(join(pkgRoot, 'dist', 'README.md')),
      false,
      'sanity: dist starts WITHOUT README.md'
    );
    const result = await versionImpl({}, context);
    assert.equal(result.success, true);
    ...
    assert.equal(
      existsSync(join(pkgRoot, 'dist', 'README.md')),
      true,
      're-stamp must restore the missing README.md before hashing'
    );
    assert.equal(
      readFileSync(join(pkgRoot, 'dist', 'README.md'), 'utf8'),
      README
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    ...
    assert.equal(
      after.version,
      '1.0.0',
      'a package with no real content change must NOT bump, even if dist was missing an asset before this run'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('BUG-BUILD-PUBLISH-DISTMANIFEST-CLOBBERED-001: a CORRUPTED dist/package.json (as `build` alone would leave it) is re-stamped before hashing, so an unchanged package does NOT spuriously bump', async (t) => {
  ...
  try {
    ...
    const distFilesCorrupted = {
      'package.json': JSON.stringify(clobberedDistManifest),
      'src/index.js': 'export const x = 1;\n',
    };
    const { pkgRoot, context } = makeProject({
      rootDir,
      ...
      projectRoot: 'packages/pkg-b',
      srcPkg,
      distFiles: distFilesCorrupted,
    });
    makeFiles(join(rootDir, 'packages/pkg-dep'), {
      'package.json': JSON.stringify({
        name: '@adhd/pkg-dep',
        version: '1.0.0',
      }),
    });
    context.projectsConfigurations.projects['pkg-dep'] = {
      root: 'packages/pkg-dep',
    };
    writePublishedState(rootDir, {
      '@adhd/pkg-b': {
        version: '1.0.0',
    ...
    assert.deepEqual(
      networkCalls(state),
      [],
      'a cache hit (after correct re-stamping) must never touch npm or tar'
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    );
    assert.equal(
      after.version,
      '1.0.0',
      'a package with no real content change must NOT bump, even if dist was corrupted before this run'
    );
    // And the corruption itself must actually be fixed on disk, not merely worked around in-memory.
    const distPkgNow = JSON.parse(
      readFileSync(join(pkgRoot, 'dist', 'package.json'), 'utf8')
    );
    assert.equal(
      distPkgNow.files,
      undefined,
      're-stamp must strip the source "files" allowlist'
    );
    assert.deepEqual(
      distPkgNow.bin,
      { 'pkg-b': 'src/cli/run.js' },
      're-stamp must rebase bin'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('cache HIT, changed: ZERO network calls, still bumps correctly', async (t) => {
  ...
  try {
    ...
    writePublishedState(rootDir, {
      '@adhd/pkg-b': {
        version: '1.0.0',
        normalizedHash: normalizedHash(oldPublishedDir),
        publishedIntegrity: 'sha512-whatever',
      },
    });
    ...
    assert.deepEqual(
      networkCalls(state),
      [],
      'a cache hit must NEVER touch npm or tar, even when the package DID change (Deliverable 2)'
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    );
    assert.equal(
      after.version,
      '1.0.1',
      'a real code change vs the cached hash must still bump'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('cache HIT but source already ahead of the cached version: ZERO network, treated as "release pending"', async (t) => {
  ...
  try {
    ...
    writePublishedState(rootDir, {
      '@adhd/pkg-b': {
        version: '1.0.0',
        normalizedHash: 'sha256:irrelevant',
        publishedIntegrity: 'sha512-whatever',
      },
    });
    ...
    assert.deepEqual(
      networkCalls(state),
      [],
      'must never touch the network just to notice source is already ahead of the cache'
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    );
    assert.equal(
      after.version,
      '1.0.1',
      'must be left exactly as-is — release already pending'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('BUG-005 REGRESSION-LOWER: source version BEHIND the cache\'s recorded published version is a HARD FAILURE, not "release pending"', async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.success,
      false,
      'a version regression relative to the published-state cache must fail, never silently pass'
    );
    assert.deepEqual(
      networkCalls(state),
      [],
      'detecting the regression itself must still be zero-network — the cache alone is enough to know'
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    );
    assert.equal(
      after.version,
      '1.0.0',
      'a failed regression gate must never rewrite the (already-wrong) source version'
    );
...
test("BUG-005 boundary: source version EQUAL to the cache's recorded version is NOT a regression (falls through to the normal hash-compare path)", async (t) => {
  ...
  try {
    ...
    writePublishedState(rootDir, {
      '@adhd/pkg-b': {
        version: '1.0.0',
        normalizedHash: normalizedHash(join(pkgRoot, 'dist')),
        publishedIntegrity: 'sha512-whatever',
      },
    });
    ...
    assert.equal(
      result.success,
      true,
      'an EQUAL version is never a regression — this must reach the hash-compare path and succeed normally'
    );
    ...
    assert.equal(
      after.version,
      '1.0.0',
      'unchanged content vs published -> no bump'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('BUG-005 boundary: source version AHEAD of the cache\'s recorded version is the legitimate "release pending" case (unchanged behavior)', async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.success,
      true,
      'source ahead of the cache is a legitimate pending release, not a regression'
    );
    assert.deepEqual(networkCalls(state), []);
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
...
test('cache HIT, integrity fast path used at backfill time is directly consumable later with zero network (end-to-end: miss -> populate -> hit)', async (t) => {
  ...
  try {
    ...
    const state1 = newState({
      publishedVersions: ['1.0.0'],
      publishedIntegrity: matchingIntegrity,
    });
    ...
    assert.ok(
      ...
      'backfill must have populated published-state.json'
    );
    const tarCalls1 = state1.calls.filter((c) => c.cmd === 'tar');
    assert.deepEqual(
      tarCalls1,
      [],
      'the FAST path (integrity match) must never pull/extract a tarball, even on the populating run'
    );
    ...
    assert.equal(
      after1.version,
      '1.0.0',
      'local content matches what was just confirmed published -> no bump'
    );
    ...
    assert.deepEqual(
      networkCalls(state2),
      [],
      'the second run, against the now-warm cache, must be entirely zero-network'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('"not yet published" path (cache miss -> backfill -> still pending): reconciles the internal range on-disk, never via the deps-plugin executors, and writes NO cache entry', async (t) => {
  ...
  try {
    ...
    makeFiles(join(rootDir, 'packages/pkg-a'), {
      'package.json': JSON.stringify({ name: '@adhd/pkg-a', version: '1.0.0' }),
    });
    context.projectsConfigurations.projects['pkg-a'] = {
      root: 'packages/pkg-a',
    };
    ...
    assert.equal(
      state.eslintCalls.length,
      0,
      'DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): must NEVER invoke the deps-plugin sync/check executors — reconciliation is on-disk only'
    );
    assert.equal(
      existsSync(publishedStatePath(rootDir)),
      false,
      '"pending" (never published) must never write a cache entry'
    );
...
test('dry run: reconciliation is read-only on-disk, NEVER writes package.json, and never invokes the deps-plugin executors', async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.success,
      true,
      'a dry run must never fail — reconcileInternalRangesFromDisk only logs on a dry run, never throws/fails'
    );
    assert.equal(
      state.eslintCalls.length,
      0,
      'DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): dry run must NEVER invoke the deps-plugin sync/check executors'
    );
    const after = readFileSync(join(pkgRoot, 'package.json'), 'utf8');
    assert.equal(after, before, 'dry run must never write package.json');
  } finally {
...
test('ADHD_NX_VERSION_DRY_RUN=1 env var forces dry-run behavior even when options.dryRun is NOT set (BUG-NX-RUNMANY-DRYRUN-NOT-PROPAGATED-TO-DEPENDENCY-TASKS-001 mitigation)', async (t) => {
  ...
  try {
    ...
    assert.equal(
      after,
      before,
      'env-var dry run must never write, even though a real code change would otherwise bump'
    );
    assert.equal(
      state.eslintCalls.length,
      0,
      'DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): env-var dry run must NEVER invoke the deps-plugin sync/check executors'
    );
  } finally {
...
test('published (cache miss, backfill) + range-only drift vs published tarball: does NOT bump, but DOES reconcile the range (no cascade, no false positive)', async (t) => {
  ...
  try {
    ...
    const publishedPkg = {
      ...
      main: './index.js',
      dependencies: { '@adhd/pkg-a': '^1.0.0' },
    }; // only the internal range is stale
    const { pkgRoot, context } = makeProject({
      rootDir,
      name: 'pkg-b',
      projectRoot: 'packages/pkg-b',
    ...
    makeFiles(join(rootDir, 'packages/pkg-a'), {
      'package.json': JSON.stringify({ name: '@adhd/pkg-a', version: '1.1.0' }),
    });
    context.projectsConfigurations.projects['pkg-a'] = {
      root: 'packages/pkg-a',
    };
    const state = newState({
      publishedVersions: ['1.0.0'],
      publishedFiles: {
        'package.json': JSON.stringify(publishedPkg),
        'index.js': 'export const x = 1;\n',
      },
    });
    t.mock.method(child_process, 'spawnSync', makeSpawnSyncMock(state));
    ...
    assert.equal(
      after.version,
      '1.0.0',
      'a range-only diff vs published must NOT bump the own version'
    );
    assert.equal(
      state.eslintCalls.length,
      0,
      'DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): must NEVER invoke the deps-plugin sync/check executors — the internal range is already correct on disk, reconciled via reconcileInternalRangesFromDisk alone'
    );
    assert.ok(
      existsSync(publishedStatePath(rootDir)),
      'the backfill must have populated the cache'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('published (cache miss, backfill) + REAL code drift: DOES bump, then reconciles ranges afterward', async (t) => {
  ...
  try {
    ...
    assert.equal(
      after.version,
      '1.0.1',
      'a genuine code change must still bump (unchanged pre-existing behavior)'
    );
    assert.equal(
      state.eslintCalls.length,
      0,
      'DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): must NEVER invoke the deps-plugin sync/check executors, even after a real bump'
    );
    const cached = JSON.parse(
      readFileSync(publishedStatePath(rootDir), 'utf8')
    );
    assert.equal(
      cached['@adhd/pkg-b'].version,
      '1.0.0',
      'cache records the PUBLISHED version (pre-bump), not the new local one'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('published (cache miss, backfill) + REAL code drift: generates a CHANGELOG.md entry via real `nx release changelog`, --first-release when no prior entry commit is known', async (t) => {
  ...
  try {
    ...
    assert.equal(result.success, true);
    assert.equal(
      state.changelogCalls.length,
      1,
      'nx release changelog must be invoked exactly once'
    );
    const args = state.changelogCalls[0];
    assert.deepEqual(
      args.slice(0, 3),
      ['release', 'changelog', '1.0.1'],
      'must target the NEW version just decided'
    );
    assert.ok(
      args.includes('--projects') && args.includes('pkg-b'),
      'must scope to THIS project only'
    );
    assert.ok(
      args.includes('--first-release'),
      'no prior changelog commit known -> --first-release, not a fabricated --from'
    );
    assert.ok(
      !args.includes('--from'),
      'must not pass --from when no boundary is known'
    );
    assert.ok(
      args.includes('--git-commit') &&
        args[args.indexOf('--git-commit') + 1] === 'false',
      'must never let nx auto-commit'
    );
    assert.ok(
      args.includes('--git-tag') &&
        args[args.indexOf('--git-tag') + 1] === 'false',
      'must never let nx auto-tag'
    );
    assert.ok(
      !args.includes('--dry-run'),
      'a real (non-dry-run) bump must NOT pass --dry-run to the changelog call'
    );
    assert.ok(
    ...
    assert.ok(
      state.calls.some(
        (c) => c.cmd === process.execPath && c.args[0] === NX_BIN
      ),
...
test("when a prior commit touched this project's CHANGELOG.md, uses --from=<that sha> instead of --first-release", async (t) => {
  ...
  try {
    ...
    assert.ok(
      !args.includes('--first-release'),
      'a known boundary must not ALSO claim first-release'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('dry run with real code drift (cache miss -> backfill still runs, but never writes package.json): previews the changelog via --dry-run', async (t) => {
  ...
  try {
    ...
    assert.equal(
      state.changelogCalls.length,
      1,
      'dry run still PREVIEWS the changelog for visibility'
    );
    assert.ok(
      state.changelogCalls[0].includes('--dry-run'),
      'the preview call must itself be a dry run — never writes CHANGELOG.md'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('changelog generation failure fails the whole version task (bump already landed, still surfaced as a failure) and never reaches range reconciliation', async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.success,
      false,
      'a real changelog-generation failure must fail the task'
    );
    assert.equal(
      state.eslintCalls.length,
      0,
      'the deps-plugin executors are never invoked regardless (DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b))'
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    );
    assert.equal(
      after.version,
      '1.0.1',
      'the version write itself already landed before the changelog step — this is a surfaced failure, not a rollback'
    );
    assert.equal(
      after.dependencies['@adhd/pkg-a'],
      '^1.0.0',
      'must fail FAST — reconcileInternalRangesFromDisk must never run once changelog generation fails, so the genuinely-stale range must be left untouched'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
  ...
  try {
    ...
    try {
      ...
      assert.equal(
        result.success,
        false,
        'a real reconcileInternalRangesFromDisk write failure must propagate as an overall executor failure'
      );
      assert.equal(
        state.eslintCalls.length,
        0,
        'still never invokes the deps-plugin executors — the failure is entirely within the on-disk reconciliation path'
      );
    } finally {
...
test('backfill failure (network error reconciling a cache miss) leaves version untouched and still reconciles ranges', async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.success,
      true,
      'a backfill failure must not fail the whole task — it leaves version as-is for manual verification'
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    );
    assert.equal(
      after.version,
      '1.0.0',
      'must be left untouched on a backfill error'
    );
    assert.equal(
      existsSync(publishedStatePath(rootDir)),
      false,
      'an errored backfill must never write a cache entry'
    );
    assert.equal(
      state.eslintCalls.length,
      0,
      'DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): range reconciliation still runs after a backfill error, but never via the deps-plugin executors'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test("reconciles an internal @adhd/* range directly from the dependency's on-disk package.json, without ever consulting the deps-plugin executors", async (t) => {
  ...
  try {
    ...
    assert.equal(
      state.eslintCalls.length,
      0,
      'DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b): the deps-plugin executors must never be consulted for this'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('dry run: reports the internal-range fix it would make but never writes package.json', async (t) => {
  ...
  try {
    ...
    assert.equal(
      after,
      before,
      'dry run must never write the internal-range fix either'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('DEBT-002 #1 (corrected): a CONCRETE range on an @adhd/* dependency absent from the workspace project graph is ALLOWED — a legitimate external @adhd-scoped package (e.g. @adhd/sox-graph-store) — version proceeds normally', async (t) => {
  ...
  try {
    const localPkg = {
      ...
      version: '1.0.0',
      main: './index.js',
      dependencies: { '@adhd/sox-graph-store': '^0.3.0' },
    };
    const { pkgRoot, context } = makeProject({
      rootDir,
      name: 'pkg-b',
    ...
    assert.equal(
      result.success,
      true,
      'a concrete external @adhd/* range must never be treated as an unresolvable hazard'
    );
    const after = JSON.parse(
      readFileSync(join(pkgRoot, 'package.json'), 'utf8')
    );
    assert.equal(
      after.dependencies['@adhd/sox-graph-store'],
      '^0.3.0',
      'the external range must be left exactly as authored'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('DEBT-002 #1 (corrected): a surviving "workspace:*" range on an @adhd/* dependency absent from the workspace project graph is STILL a hard failure (the real hazard)', async (t) => {
  ...
  try {
    const localPkg = {
      ...
      version: '1.0.0',
      main: './index.js',
      dependencies: { '@adhd/not-a-real-project': 'workspace:*' },
    };
    const { pkgRoot, context } = makeProject({
      rootDir,
      name: 'pkg-b',
    ...
    assert.equal(
      after,
      before,
      'a failed gate must never have written a (still-unresolvable) version bump to the source package.json'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('DEBT-002 #5: a dep declared with DIFFERENT range prefixes in `dependencies` (^) vs `peerDependencies` (~) each keep their OWN prefix — no cross-field contamination', async (t) => {
  ...
  try {
    ...
    assert.equal(
      after.dependencies['@adhd/pkg-a'],
      '^2.0.0',
      'dependencies keeps its OWN ^ prefix, updated to the current on-disk version'
    );
    assert.equal(
      after.peerDependencies['@adhd/pkg-a'],
      '~2.0.0',
      "peerDependencies keeps its OWN ~ prefix — must NOT be contaminated by dependencies' ^ prefix"
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test("DEBT-002 #5 dry run: reports BOTH fields' fixes with their own prefixes, never writes package.json", async (t) => {
  ...
  try {
    ...
    assert.equal(
      after,
      before,
      'a dry run must never write package.json, even with multi-field drift'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('changelog scrub: a package declaring `changelogVocabulary` has its GENERATED CHANGELOG.md scrubbed in place (real executor, real fs, real dynamic import)', async (t) => {
  ...
  try {
    ...
    assert.equal(
      result.success,
      true,
      'a successful generate+scrub must not fail the task'
    );
    const after = readFileSync(changelogPath, 'utf8');
    assert.ok(
      !/sqlite/i.test(after),
      `the generated entry must carry no forbidden term\n---\n${after}`
    );
    assert.ok(
      /store-engine/i.test(after),
      'the forbidden term must be rewritten to its neutral paraphrase, not deleted'
    );
    assert.ok(
      after.startsWith('## 1.0.1'),
      'the entry structure must survive the scrub'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('changelog scrub: a package with NO `changelogVocabulary` declaration is left untouched (other packages legitimately name such terms)', async (t) => {
  ...
  try {
    ...
    assert.equal(
      ...
      entry,
      'no declaration -> the historical record must be byte-identical'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('changelog scrub: a declared-but-BROKEN policy fails the task loudly (never silently ships a dirty changelog)', async (t) => {
  ...
  try {
    ...
    makeFiles(pkgRoot, {
      'project.json': JSON.stringify({
        name: 'pkg-b',
        changelogVocabulary: 'tools/missing-policy.mjs',
      }),
    });
    const changelogPath = join(pkgRoot, 'CHANGELOG.md');
    ...
    assert.equal(
      result.success,
      false,
      'a declared policy that cannot be loaded must fail the task, never be silently skipped'
    );
  } finally {
    rmSync(rootDir, { recursive: true, force: true });
  }
...
test('changelog policy (single source of truth): the REAL @adhd/backlog policy scrubs every term BOTH gates enforce, and the live incident lines', async () => {
  ...
  const policy = await import(
    pathToFileURL(
      join(repoRoot, 'entrypoint/backlog/tools/vocabulary-policy.mjs')
    ).href
  );
  ...
  for (const term of [...policy.SRC_TERMS, ...policy.TARBALL_TERMS]) {
    ...
    assert.ok(
      term.re.test(sample),
      `test sample for '${term.name}' must match that gate's own detection regex`
    );
    const scrubbed = policy.scrubChangelog(sample);
    assert.deepEqual(
      policy.findBannedHits(scrubbed),
      [],
      `scrubChangelog must neutralise '${term.name}': ${JSON.stringify(
        scrubbed
      )}`
    );
    assert.equal(
      policy.scrubChangelog(scrubbed),
      scrubbed,
  ...
  for (const line of incidentLines) {
    assert.deepEqual(
      policy.findBannedHits(policy.scrubChangelog(line)),
      [],
      `incident line must scrub clean: ${line}`
    );
  }
});
