...
3. **Reconciles its own declared internal ranges (every run, regardless of step 1's outcome):** after the bump decision, `version` reconciles THIS package's declared internal `@adhd/*` dependency ranges to the workspace's now-settled versions by reading every declared internal dependency's `package.json` **directly off disk** (`reconcileInternalRangesFromDisk` in `impl.js`). It writes **only this project's own** `package.json`, never a sibling's. **DEBT-BUILD-VERSION-SYNCDEPS-REDUNDANT-001 (b):** this step used to ALSO call `tools/nx-plugins/deps/executors/sync/impl.js` (fix) / `.../check/impl.js` (read-only, `--dryRun`) in-process first — running the full `@nx/dependency-checks` ESLint rule a SECOND time for the same project on every `version` run. That call has been removed: `version`'s own `dependsOn: ["build", "assets", "^version"]` already guarantees `build` → `lint` → `sync-deps` (`nx.json` targetDefaults) ran the real `sync-deps` for this exact project earlier in the same task-graph invocation, so nothing was left for `version` itself to re-check there — only the on-disk reconciliation (correcting for the ESLint rule's project graph going mid-run-stale, see below) is genuinely `version`-specific.
