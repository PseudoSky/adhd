...
// ---------------------------------------------------------------------------
// BUG-039's cross-process writers harness previously lived here, behind
// `describe.skip`. It has been relocated to
// `src/write/cross-process-write-safety.spec.ts` — repointed onto the v2
// `createIssue`/`queryIssues` surface (identity is now a DB-generated `uid`;
// there is no `humanId`/`family`/`repo`-string identity to reproduce the old
// defect against) and UN-skipped (AGENTS.md's "Live testing is mandatory —
// no silent gating" forbids a `describe.skip` cross-process harness; that is
// exactly why this defect went unproven for so long). `src/store/` — this
// file's own directory — is scheduled for deletion, which is the reason for
// the move: a gate left here would have been deleted along with the code it
// exists to gate.
// ---------------------------------------------------------------------------
