 ...
 * `mode: 'immediate'` transaction, compute a full new metadata object in
 ...
 * pattern).
 *
 * `mode: 'immediate'` (not the default deferred `BEGIN`) is load-bearing: a
 ...
 * transaction start, so a second process's own `mode: 'immediate'` call
 * blocks (up to `busy_timeout`) rather than interleaving (DESIGN.md §3).
 *
 * The transaction is wrapped in `withImmediateRetry` (bounded, jittered
 * exponential backoff on `SQLITE_BUSY`/`SQLITE_BUSY_TIMEOUT` only —
