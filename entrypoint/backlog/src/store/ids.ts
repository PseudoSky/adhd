 ...
 * issued the same `-NNN` suffix. Same `.immediate()`-under-contention failure
 * mode as `mutateMetadata` applies here too, so this shares the identical
 * bounded-retry wrapper (DEBT-BACKLOG-CONCURRENCY-BUSY-RETRY-001).
 */
