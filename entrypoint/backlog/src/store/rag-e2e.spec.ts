...
type DupHandle = TestIssueStore & {
  ...
  search: { backend: StoreSearchBackend; embedQuery(text: string): Promise<Float32Array> };
};
...
async function openWithBackend(label: string): Promise<OpenedStore> {
  ...
  const result = await bootstrapSemanticBackend(store as unknown as GraphBacklogStore, { embedding: EMBEDDING });
  if (!result.ok) {
  ...
  const vec = await openTursoVectorStore(store.adapter, { dim: backend.dim, modelId: backend.modelId });
  const handle: DupHandle = {
    ...store,
    embedding: backend,
    search: { backend: new StoreSearchBackend(vec, store.graph), embedQuery: (text: string) => backend.embedQuery(text) },
  };
...
async function file(handle: DupHandle, projectUid: string, title: string, body: string, opts?: { force?: boolean }): Promise<ICreateIssueResult> {
  return createIssue(handle, { project: projectUid, title, body, by: 'filer', awaitEmbed: true, ...(opts?.force ? { duplicateAction: 'force' } : {}) });
}
...
const FILLER_ISSUES: ReadonlyArray<readonly [string, string]> = [
  ['Rate limiter resets counters every fixed window', 'Requests beyond the quota return 429 until the next window boundary.'],
  ['Config loader merges layered YAML files', 'Later layers override earlier ones key by key, arrays are replaced wholesale.'],
  ['Retry policy uses exponential backoff with jitter', 'Jitter prevents synchronized retry storms across many clients.'],
  ['Log rotation compresses files older than one day', 'Compressed logs are kept for thirty days then deleted.'],
  ['Feature flags are evaluated per request', "A flag's value can change without redeploying anything."],
  ['Health check endpoint reports dependency status', 'Each dependency is polled on its own interval.'],
  ['Session tokens expire after inactivity', 'A refresh call extends the expiry silently.'],
  ['Pagination cursors encode the last seen sort key', 'A cursor from one query is never valid for a different sort order.'],
  ['Webhook delivery retries with capped attempts', 'After the cap, the event moves to a dead-letter queue for manual replay.'],
  ['Thumbnail generation resizes images on upload', 'The original file is kept alongside every generated size.'],
];
 ...
 * asserts on.
 */
async function seedFillerCorpus(handle: DupHandle, projectUid: string): Promise<void> {
  for (const [title, body] of FILLER_ISSUES) {
    const result = await createIssue(handle, { project: projectUid, title, body, by: 'filer', awaitEmbed: true, duplicateAction: 'force' });
    if (!result.created) throw new Error(`seedFillerCorpus: filler issue "${title}" unexpectedly failed to write: ${JSON.stringify(result)}`);
  }
}
...
describe('RAG §8 DoD — real fastembed + real Turso vectors', () => {
  ...
  it('DoD#2 — an item created through the real write path is retrievable by MEANING, not keywords', async () => {
    ...
    const target = await file(handle, projectUid, 'Vector index refuses a dimension mismatch',
      'Inserting an embedding whose length differs from the configured space dimension is rejected structurally.', { force: true });
  ...
  it('DoD#2b — the vector is durably readable back for the created node (on-write embedding round-trip)', async () => {
    opened = await openWithBackend('rag-e2e-dod2b');
    const { handle, store, backend, projectUid } = opened;

    const item = await file(handle, projectUid, 'Durable vector round-trip', 'The embedding written on create can be read back for this node.');
    if (!item.created || !item.uid) throw new Error(`expected the issue to be created, got ${JSON.stringify(item)}`);
  ...
  it('DoD#3 — the duplicate gate catches a PARAPHRASED duplicate that shares no meaningful title tokens', async () => {
    opened = await openWithBackend('rag-e2e-dod3');
    const { handle, projectUid } = opened;
    await seedFillerCorpus(handle, projectUid);

    await file(handle, projectUid, 'Database connection pool leaks under sustained load',
    ...
    const dup = await file(handle, projectUid, 'Connections are never released back to the pool when saturated',
    ...
    expect(dup.duplicateCandidates?.[0]?.title).toBe('Database connection pool leaks under sustained load');
  }, E2E_TIMEOUT);
  ...
  it('DoD#3b — NEGATIVE CONTROL: an unrelated item is NOT flagged as a duplicate (the floor has teeth)', async () => {
    ...
    const unrelated = await file(handle, projectUid, 'Bananas are a tropical fruit',
  ...
  it('DoD#6 — the duplicate scan is scoped to the target project: an identical item in another project never appears', async () => {
    opened = await openWithBackend('rag-e2e-dod6');
    const { handle, store, projectUid } = opened;

    await file(handle, projectUid, 'Vector index refuses a dimension mismatch',
      'Inserting an embedding whose length differs from the configured space dimension is rejected structurally.');

    const { projectUid: otherProjectUid } = await seedProject(store, 'rag-e2e-dod6-other-project');
    ...
    const other = await file(handle, otherProjectUid, 'Vector index refuses a dimension mismatch',
      'Inserting an embedding whose length differs from the configured space dimension is rejected structurally.');
  ...
  it('enableSemanticSearchFromConfig is silent and leaves RAG unconfigured when disabled', async () => {
    bareDir = freshTmpDir('rag-e2e-disabled');
    bareStore = await openTestIssueStore(join(bareDir, 'backlog.db'));
    const backend = await enableSemanticSearchFromConfig(bareStore as unknown as GraphBacklogStore, {
      enabled: false,
      provider: 'fastembed',
      model: MODEL,
    });
  ...
  it('enableSemanticSearchFromConfig installs the seam when enabled', async () => {
