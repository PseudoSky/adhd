...
describe('transitionStatusNode — status-vocabulary teeth gate', () => {
  ...
  beforeEach(async () => {
    tmp = await openTmpStore('lifecycle-spec');
  });
  ...
  it('rejects a transition into FIXED with zero citations', async () => {
    const created = await createItemNode(tmp.store, { family: 'BUG-GATE', title: 't', body: 'b', repo: REPO });
    ...
    const stillOpen = await tmp.store.graph.getNode(created.item.nodeId);
  ...
  it('accepts a transition into FIXED WITH a citation, and clears any prior claim', async () => {
    const created = await createItemNode(tmp.store, { family: 'BUG-GATE', title: 't', body: 'b', repo: REPO });
    const result = await transitionStatusNode(tmp.store, REPO, created.item.humanId, 'FIXED', {
      by: 'implementer:x',
  ...
  it('a citation already attached via addCitation satisfies the gate without an inline one', async () => {
    ...
    const result = await transitionStatusNode(tmp.store, REPO, created.item.humanId, 'FIXED', { by: 'implementer:x' });
    expect(result.status).toBe('FIXED');
    expect(result.citations).toHaveLength(1);
  });

  it('a non-terminal transition (e.g. BLOCKED) never requires evidence', async () => {
    const created = await createItemNode(tmp.store, { family: 'BUG-GATE', title: 't', body: 'b', repo: REPO });
    const result = await transitionStatusNode(tmp.store, REPO, created.item.humanId, 'BLOCKED', { by: 'x' });
