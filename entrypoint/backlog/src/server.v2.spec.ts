...
function parseCliTable(help: string): { apigen: string[]; special: string[] } {
  ...
  for (const line of tableBlock.split('\n')) {
    ...
    // identity `entry.cliCommand` (`proj.cli.path.join(' ')`, server.ts:216)
    ...
    const canonicalHead = head.startsWith('[') && head.endsWith(']') ? head.slice(1, -1) : head;
    apigen.push(next && !next.startsWith('{') ? `${canonicalHead} ${next}` : canonicalHead);
  }
