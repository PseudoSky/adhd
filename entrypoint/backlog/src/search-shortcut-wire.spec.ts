...
beforeAll(() => {
  ...
  okData(['upsert-project', '--input', JSON.stringify({ name: PROJECT_NAME, by: 'search-wire.spec' })]);

  const target = okData([
    'upsert-component',
    '--input',
    JSON.stringify({ project: PROJECT_NAME, name: 'wire-target', path: 'packages/wire/target', by: 'search-wire.spec' }),
  ]);
  targetComponentUid = target['uid'] as string;
  const sibling = okData([
    'upsert-component',
    '--input',
    JSON.stringify({ project: PROJECT_NAME, name: 'wire-sibling', path: 'packages/wire/sibling', by: 'search-wire.spec' }),
  ]);

  planUid = okData([
    'create',
    '--input',
    JSON.stringify({ title: 'search wire plan', body: 'plan body', project: PROJECT_NAME, kind: 'plan', by: 'search-wire.spec', duplicateAction: 'force' }),
  ])['uid'] as string;
  memberUid = okData([
    'create',
    '--input',
    JSON.stringify({ title: 'search wire plan member', body: 'member body', project: PROJECT_NAME, by: 'search-wire.spec', duplicateAction: 'force' }),
  ])['uid'] as string;
  outsiderUid = okData([
    'create',
    '--input',
    JSON.stringify({ title: 'search wire outsider', body: 'outsider body', project: PROJECT_NAME, by: 'search-wire.spec', duplicateAction: 'force' }),
  ])['uid'] as string;
  okData(['relate', '--input', JSON.stringify({ sourceUid: memberUid, targetUid: planUid, rel: 'part_of', action: 'add', by: 'search-wire.spec' })]);

  inTargetUid = okData([
    'create',
    '--input',
    JSON.stringify({ title: 'search wire in target', body: 'in target body', project: PROJECT_NAME, component: targetComponentUid, by: 'search-wire.spec', duplicateAction: 'force' }),
  ])['uid'] as string;
  inSiblingUid = okData([
    'create',
    '--input',
    JSON.stringify({ title: 'search wire in sibling', body: 'in sibling body', project: PROJECT_NAME, component: sibling['uid'], by: 'search-wire.spec', duplicateAction: 'force' }),
  ])['uid'] as string;
}, 180_000);
...
describe('backlog search --plan / --project-path at the wire (real built bin)', () => {
  ...
  it('--plan reaches the real mounted schema (no additionalProperty rejection) and returns exactly the part_of member', () => {
    ...
    expect(run.status, `search --plan exited ${String(run.status)}: ${run.stderr}`).toBe(0);
    const body = JSON.parse(run.stdout.trim().split('\n').filter(Boolean).pop() ?? '{}') as Record<string, unknown>;
    expect(body['ok'], `search --plan returned an error arm: ${JSON.stringify(body)}`).toBe(true);
    ...
    const uids = (data['items'] as Array<Record<string, unknown>>).map((i) => i['uid']);
  ...
  it('--project-path reaches the real mounted schema and returns exactly the matching component\'s issue', () => {
    const run = runBin(['search', 'in target', '--project-path', 'packages/wire/target']);
    expect(run.status, `search --project-path exited ${String(run.status)}: ${run.stderr}`).toBe(0);
    const body = JSON.parse(run.stdout.trim().split('\n').filter(Boolean).pop() ?? '{}') as Record<string, unknown>;
    expect(body['ok'], `search --project-path returned an error arm: ${JSON.stringify(body)}`).toBe(true);
    const data = body['data'] as Record<string, unknown>;
    expect(Object.keys(data)).toContain('items');
    const uids = (data['items'] as Array<Record<string, unknown>>).map((i) => i['uid']);
