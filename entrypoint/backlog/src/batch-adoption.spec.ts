 ...
 * case in `batch-adoption.e2e.ts`); they are the contract a mocked version must satisfy
 * without touching a subprocess or a real model.
 *
 * Resource lane: proc — starts the real startBacklogServer({transport:"http"}) (a bound port) and drives it with real fetch.
 */
import { describe, it } from 'vitest';

describe("mocked: batch-adoption", () => {
  it.todo("mocked: creates real items via a real fan-out, and a per-item validation failure rejects without aborting the batch");
});
