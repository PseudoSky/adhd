...
describe('runBacklogCli — live CLI mount, real spawned dist/index.js bin, temp-scoped store', () => {
  ...
  it('"batch action --operation backlog/create --items […]" fans out via the real CLI to real client.ts create, and both items persist independently (BUG-018 / batch-CLI wiring)', () => {
    ...
    // BUG-APIGEN-CLI-002: batch's control-plane fields (`operation`, `items`,
    // `concurrency`, `mode`, `onItemError`, `itemTimeoutMs`) are nested under
    // ONE `--input <json>` flag now — matching every other apigen-mounted
    // operation's single-JSON-blob CLI convention, not per-field flags. See
    // `apigen-core-client`'s `batch.ts`:`branchInputSchema` and
    // `batch-plugin-cli-live-dispatch.spec.ts` for the same, already-proven
    // shape.
    const res = runBin(
      ['batch', 'action', '--input', JSON.stringify({ operation: 'backlog/create', items, concurrency: 2, onItemError: 'continue' })],
      adhdRoot
    );
    expect(res.status, `stderr:\n${res.stderr}\nstdout:\n${res.stdout}`).toBe(0);
