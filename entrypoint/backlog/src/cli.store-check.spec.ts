 ...
 * sibling `cli.store-check.e2e.ts`.
 *
 ...
 * case in `cli.store-check.e2e.ts`); they are the contract a mocked version must satisfy
 * without touching a subprocess or a real model.
 */
import { describe, it } from 'vitest';

describe("mocked: cli.store-check", () => {
  it.todo("mocked: exits 0 with ok:true on a store this build recognizes");
  it.todo("mocked: exits 1 with code store_vocabulary_mismatch on a foreign-vocabulary store");
});
