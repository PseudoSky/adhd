...
import { createIssue, type ICreateIssueInput } from '../../write/create-issue.js';
import { InvalidArgumentError, BacklogValidationError, IssueNotFoundError } from '../../write/errors.js';
...
import { DEFAULT_TEMPORAL_DECAY_PER_HOUR, querySimilarView, rankByFusedRelevance, resolveSimilarFilterIds } from './semantic.js';
...
interface SemanticTestStore {
  ...
  createIssueFixture(input: Omit<ICreateIssueInput, 'project' | 'by'> & { project: string }): Promise<{ uid: string; title: string }>;
  close(): Promise<void>;
}
  ...
  const handle: IQueryStoreHandle = {
    ...
    search: {
      ...
      embedQuery: async (text: string): Promise<Float32Array> => {
        ...
        if (!v) throw new Error(`SemanticTestStore: no pinned embedding for ${JSON.stringify(text)} — call pinEmbedding() first`);
        return v;
      },
    },
  };
...
describe('querySimilarView — filter.semantic ranking', () => {
  it('ranks by fused relevance descending (vec channel), best match first', async () => {
    ...
    const near = await s.createIssueFixture({ title: 'near', body: 'aaacontent-near', project: projectUid });
    const mid = await s.createIssueFixture({ title: 'mid', body: 'aaacontent-mid', project: projectUid });
    const far = await s.createIssueFixture({ title: 'far', body: 'aaacontent-far', project: projectUid });
    ...
    const items = await querySimilarView(s.handle, { view: 'similar', filter: { semantic: 'zzqueryzz' } });
  ...
  it('populates _score only when requested, and never otherwise', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const item = await s.createIssueFixture({ title: 'scored', body: 'aaascoredbody', project: projectUid });
    await s.indexIssue(item.uid, [1, 0, 0]);
    s.pinEmbedding('zzscoreprobe', [1, 0, 0]);

    const withScore = await querySimilarView(s.handle, { view: 'similar', filter: { semantic: 'zzscoreprobe' }, fields: ['uid', '_score'] });
    expect(withScore[0]?._score).toBeTypeOf('number');
    expect(withScore[0]!._score).toBeGreaterThan(0);

    const withoutScore = await querySimilarView(s.handle, { view: 'similar', filter: { semantic: 'zzscoreprobe' } });
  ...
  it('a title/body TEXT match surfaces even when its vector is farthest (SPEC.md §5a)', async () => {
    ...
    const decoys = await Promise.all(
      decoyVecs.map(async (v, i) => {
        const decoy = await s.createIssueFixture({ title: `decoy${i}`, body: `no-shared-token-here-${i}`, project: projectUid });
        await s.indexIssue(decoy.uid, [...v]);
        return decoy;
      }),
    );
    const textMatch = await s.createIssueFixture({ title: 'text winner', body: 'contains uniquetextkeyword literally', project: projectUid });
    await s.indexIssue(textMatch.uid, [0, 1, 0]); // orthogonal — the worst possible vector match
    s.pinEmbedding('uniquetextkeyword', [1, 0, 0]);

    const items = await querySimilarView(s.handle, { view: 'similar', filter: { semantic: 'uniquetextkeyword' }, limit: 3 });
    ...
    expect(items.map((i) => i.uid)).toEqual(expect.arrayContaining([decoys[0]!.uid, decoys[1]!.uid]));
  });
});
...
describe('querySimilarView — filter.anchor', () => {
  it('excludes the anchor from its own results even when it would rank #1', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const anchor = await s.createIssueFixture({ title: 'anchor-title', body: 'aaanchorbody', project: projectUid });
    const neighbor = await s.createIssueFixture({ title: 'neighbor-title', body: 'aaaneighborbody', project: projectUid });
    await s.indexIssue(anchor.uid, [1, 0, 0]);
    await s.indexIssue(neighbor.uid, [0.99, 0.01, 0]);
    s.pinEmbedding(`anchor-title\naaanchorbody`, [1, 0, 0]);

    const items = await querySimilarView(s.handle, { view: 'similar', filter: { anchor: anchor.uid } });
  ...
  it('an anchor with zero live neighbours returns [] (never the anchor itself)', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const anchor = await s.createIssueFixture({ title: 'lonely-anchor', body: 'aaalonelybody', project: projectUid });
    await s.indexIssue(anchor.uid, [1, 0, 0]);
  ...
  it('an unresolved anchor uid throws IssueNotFoundError (SPEC.md §6.1 uid-addressing convention)', async () => {
    await expect(querySimilarView(s.handle, { view: 'similar', filter: { anchor: 'not-a-real-uid' } })).rejects.toBeInstanceOf(
      IssueNotFoundError,
    );
  });
});
...
describe('querySimilarView — input validation', () => {
  it('neither anchor nor semantic given throws InvalidArgumentError', async () => {
    await expect(querySimilarView(s.handle, { view: 'similar' })).rejects.toBeInstanceOf(InvalidArgumentError);
  });
  ...
  it('no search backend configured throws InvalidArgumentError', async () => {
    ...
    await expect(querySimilarView(unconfigured, { view: 'similar', filter: { semantic: 'x' } })).rejects.toBeInstanceOf(
  ...
  it('an out-of-range limit throws BacklogValidationError', async () => {
    await expect(
      querySimilarView(s.handle, { view: 'similar', filter: { semantic: 'x' }, limit: 0 }),
    ).rejects.toBeInstanceOf(BacklogValidationError);
    await expect(
      querySimilarView(s.handle, { view: 'similar', filter: { semantic: 'x' }, limit: 1.5 }),
    ).rejects.toBeInstanceOf(BacklogValidationError);
  });
  ...
  it('an unknown field name throws BacklogValidationError, never a silent drop (card.ts:33\'s stated contract, mirrored from queryList/get)', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const item = await s.createIssueFixture({ title: 'x', body: 'aaafieldsprobebody', project: projectUid });
    await s.indexIssue(item.uid, [1, 0, 0]);
    s.pinEmbedding('zzfieldsprobe', [1, 0, 0]);

    await expect(
      querySimilarView(s.handle, { view: 'similar', filter: { semantic: 'zzfieldsprobe' }, fields: ['uid', 'titel' as never] }),
    ).rejects.toBeInstanceOf(BacklogValidationError);
  });
});
...
describe("querySimilarView — anchor fetch-size boundary (MAX_QUERY_LIMIT, off-by-one)", () => {
  it('requests limit+1 from searchRanked when anchored, uncapped at MAX_QUERY_LIMIT — instruments the real searchRanked call, delegates to the unmodified implementation', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const anchor = await s.createIssueFixture({ title: 'boundary-anchor', body: 'aaaboundarybody', project: projectUid });
    ...
    try {
      await querySimilarView(s.handle, { view: 'similar', filter: { anchor: anchor.uid }, limit: MAX_QUERY_LIMIT });
...
describe("queryIssues — view:'similar' dispatch reaches querySimilarView (SPEC.md §5a reachability)", () => {
  it('excludes the anchor from its own results through the real query() verb, not just querySimilarView called directly', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const anchor = await s.createIssueFixture({ title: 'dispatch-anchor', body: 'aaadispatchanchorbody', project: projectUid });
    const neighbor = await s.createIssueFixture({ title: 'dispatch-neighbor', body: 'aaadispatchneighborbody', project: projectUid });
    await s.indexIssue(anchor.uid, [1, 0, 0]);
    await s.indexIssue(neighbor.uid, [0.99, 0.01, 0]);
    ...
    const result = await queryIssues(s.handle, { view: 'similar', filter: { anchor: anchor.uid } });
  ...
  it('an unresolved anchor uid rejects with IssueNotFoundError, not CatalogNotFoundError (SPEC.md §6.1)', async () => {
    ...
    await expect(queryIssues(s.handle, { view: 'similar', filter: { anchor: 'not-a-real-uid' } })).rejects.toBeInstanceOf(
      IssueNotFoundError,
    );
  });
});
...
describe('querySimilarView — vector-store-filter-purity (the "resolve to ids first" hazard)', () => {
  it('a metadata filter (assignee) excludes the closer-vector wrong-assignee issue — proves the filter reached the graph, not StoreSearchBackend (which drops `metadata` silently)', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const bobIssue = await s.createIssueFixture({ title: 'bob-issue', body: 'aaabobbody', project: projectUid, assignee: 'bob' });
    const aliceIssue = await s.createIssueFixture({ title: 'alice-issue', body: 'aaaalicebody', project: projectUid, assignee: 'alice' });
  ...
  it('a project filter excludes a strictly-closer out-of-project issue (edge-scoped pushdown)', async () => {
    ...
    const inProject = await s.createIssueFixture({ title: 'in-project', body: 'aaainprojbody', project: projA });
    const outProject = await s.createIssueFixture({ title: 'out-project', body: 'aaaoutprojbody', project: projB });
  ...
  it('a component filter excludes a strictly-closer out-of-component issue', async () => {
    ...
    const rootNode = await s.writeHandle.graph.getNodeByUid(rootComponentIssue.uid);
    const ownsComponentEdges = await s.writeHandle.graph.getEdges({ dst: rootNode!.id, rel: 'owns_component' });
  ...
  it('a filter that resolves to zero candidates short-circuits to [] (never an unfiltered fallthrough)', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const item = await s.createIssueFixture({ title: 'only-issue', body: 'aaaonlybody', project: projectUid, assignee: 'bob' });
    await s.indexIssue(item.uid, [1, 0, 0]);
  ...
  it('rankByFusedRelevance itself short-circuits to [] on a defined-but-empty candidateIds set, WITHOUT an unfiltered fallthrough (defense in depth, independent of querySimilarView\'s own short-circuit)', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const item = await s.createIssueFixture({ title: 'leaked-if-broken', body: 'aaaleakbody', project: projectUid });
    await s.indexIssue(item.uid, [1, 0, 0]);
...
describe('resolveSimilarFilterIds — edge-scoped filter resolution', () => {
  it('kind/status/priority/author narrow correctly (reusing resolve.ts\'s correctly-directioned resolveEdgeScopedCandidates)', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const bug = await s.createIssueFixture({ title: 'a-bug', body: 'body', project: projectUid, kind: 'bug' });
    const feature = await s.createIssueFixture({ title: 'a-feature', body: 'body', project: projectUid, kind: 'feature' });

    const bugIds = await resolveSimilarFilterIds(s.writeHandle.graph, { kind: 'bug' });
  ...
  it('undefined when no filter dimension is given at all (the {kind:"issue"} fast path)', async () => {
    ...
    const idsEmptyFilter = await resolveSimilarFilterIds(s.writeHandle.graph, {});
  ...
  it('an unresolved project name resolves to an empty Set, not undefined (SPEC.md §6.1: unresolved read-path name ⇒ zero matches)', async () => {
    const ids = await resolveSimilarFilterIds(s.writeHandle.graph, { project: 'no-such-project' });
...
describe('rankByFusedRelevance — the shared relevance-ranking primitive (reusable by a future sort:"relevance" integration)', () => {
  it('applies the SPEC.md §5a decay default when no override is given', async () => {
    const { projectUid } = await seedProject(s.writeHandle, 'proj-a');
    const item = await s.createIssueFixture({ title: 'x', body: 'aaadecaybody', project: projectUid });
    await s.indexIssue(item.uid, [1, 0, 0]);
  ...
  it('throws InvalidArgumentError when the store has no configured search backend', async () => {
    const unconfigured: IQueryStoreHandle = { graph: s.writeHandle.graph };
    await expect(
      rankByFusedRelevance(unconfigured, { vec: Float32Array.from([1, 0, 0]), limit: 10 }),
