 ...
 * case in `meta-wire.e2e.ts`); they are the contract a mocked version must satisfy
 * without touching a subprocess or a real model.
 */
import { describe, it } from 'vitest';

describe("mocked: meta-wire", () => {
  it.todo("mocked: a list-shaped read ENCODES meta as a sibling of data, with every field present");
  it.todo("mocked: meta.offset key is present and correct when the caller pages by offset");
  it.todo("mocked: a non-list-shaped view (view:\"graph\") ENCODES no meta key at all");
});
