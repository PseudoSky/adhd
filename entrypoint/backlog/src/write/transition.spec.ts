...
import { canonicalJSONStringify, getNodeByUidTx, nowISO, sha256Hex, writeNodeTx, type ITxNodeRow } from './tx.js';
...
async function liveEdges(store: TestIssueStore, rel: string, opts: { src?: number; dst?: number }): Promise<RawEdgeRow[]> {
  ...
  const { rows } = await store.adapter.executeAll<RawEdgeRow>(`SELECT src, dst FROM edge WHERE ${clauses.join(' AND ')}`, params);
  return rows;
}
...
async function seedStatus(store: TestIssueStore, name: string, terminal: boolean): Promise<string> {
  return store.adapter.transaction(async (tx) => {
    const row = await writeNodeTx(tx, { kind: 'status', name, metadata: { terminal }, at: nowISO() });
    return row.uid;
  }, { mode: 'immediate' });
}

async function setProjectPolicy(store: TestIssueStore, projectUid: string, policy: Record<string, unknown>, path?: string): Promise<void> {
  const meta: Record<string, unknown> = { policy };
  if (path !== undefined) meta.path = path;
  await store.adapter.executeRun('UPDATE node SET meta = ? WHERE uid = ?', [JSON.stringify(meta), projectUid]);
}
...
describe('transition — status change (SPEC.md §6.3.4, real store)', () => {
  ...
  beforeEach(async () => {
    ...
    const created = await createIssue(store, {
      project: projectUid, title: 'transition target', body: 'body', by: 'filer', status: 'open',
    });
  ...
  it('swaps the LIVE has_status edge and returns fromStatus/toStatus, given a note (policy default requires one)', async () => {
    const outcome = await transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress', note: 'starting work' });
  ...
  it('NoteRequiredError when no note is given (project_policy.transition_requires_note defaults true)', async () => {
    await expect(transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress' })).rejects.toThrow(NoteRequiredError);
  ...
  it('transition_requires_note:false permits a note-less transition', async () => {
    await setProjectPolicy(store, projectUid, { transitionRequiresNote: false });
    const outcome = await transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress' });
    expect(outcome.toStatus).toBe('in-progress');
  });
  ...
  it('a terminal toStatus stamps closedAt on BOTH the outcome and issue.meta.metadata.closedAt', async () => {
    ...
    const outcome = await transition(store, { uid: issueUid, by: 'closer', toStatus: 'done', note: 'shipped' });
  ...
  it('CitationRequiredError: citation_required:true + a terminal toStatus + zero citations', async () => {
    ...
    await expect(transition(store, { uid: issueUid, by: 'closer', toStatus: 'done', note: 'shipped' })).rejects.toThrow(CitationRequiredError);
  });
  ...
  it('CitationUnverifiableError: a citation file that does not resolve, with citation_requires_sha true (default) — nothing is written', async () => {
    ...
    await expect(
      transition(store, {
        uid: issueUid, by: 'closer', toStatus: 'in-progress', note: 'trying',
    ...
    const citationEdges = await liveEdges(store, 'has_citation', { src: issueRowid });
  ...
  it('a REAL, resolvable citation writes a citation node + has_citation edge with a genuine (non-"unverified") sha', async () => {
    ...
    const outcome = await transition(store, {
      uid: issueUid, by: 'closer', toStatus: 'in-progress', note: 'citing real evidence',
    ...
    const { rows: citationRows } = await store.adapter.executeAll<{ uid: string }>('SELECT uid FROM node WHERE rowid = ?', [citationEdges[0].dst]);
  ...
  it('transition.sha is a real sha256 over the canonical {target_uid, from, to, agent, note, at} record — recomputed and compared byte-for-byte', async () => {
    const outcome = await transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress', note: 'checking sha' });
    const transitionRow = await readNode(store, outcome.transitionUid);
    expect(transitionRow?.kind).toBe('transition');

    const recomputed = sha256Hex(canonicalJSONStringify({
      target_uid: issueUid, from: 'open', to: 'in-progress', agent: 'closer', note: 'checking sha',
  ...
  it('CatalogNotFoundError for a uid-shaped toStatus that does not resolve — minting never applies to a uid', async () => {
    await expect(transition(store, { uid: issueUid, by: 'closer', toStatus: '11111111-1111-4111-8111-111111111111', note: 'x' }))
  ...
  it('allowedStatuses restriction rejects a disallowed toStatus name with InvalidArgumentError', async () => {
    await setProjectPolicy(store, projectUid, { allowedStatuses: ['open', 'in-progress'] });
    await expect(transition(store, { uid: issueUid, by: 'closer', toStatus: 'wontfix', note: 'nope' })).rejects.toThrow(InvalidArgumentError);
  });
  ...
  it('§2 requiredFields: a project declaring "note" required (independent of transitionRequiresNote) rejects a note-less call — nothing written; the identical call WITH a note still succeeds under the SAME policy', async () => {
    ...
    await setProjectPolicy(store, projectUid, { transitionRequiresNote: false, requiredFields: ['note'] });
    await expect(transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress' })).rejects.toThrow(InvalidArgumentError);
    const edges = await liveEdges(store, 'has_status', { src: issueRowid });
    expect(edges).toHaveLength(1); // nothing written — status unchanged

    const outcome = await transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress', note: 'satisfies the policy' });
    expect(outcome.toStatus).toBe('in-progress');
  });
  ...
  it('§2 requiredFields: a project declaring a field OUTSIDE transition\'s input surface (e.g. "priority") does not brick every transition — that role belongs to createIssue/update, not this verb', async () => {
    ...
    const outcome = await transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress', note: 'unaffected by an unrelated required field' });
    expect(outcome.toStatus).toBe('in-progress');
  });

  it('IssueNotFoundError for a uid that never resolved to a live issue at all', async () => {
    await expect(transition(store, { uid: 'not-a-real-uid', by: 'closer', toStatus: 'open', note: 'x' })).rejects.toThrow(IssueNotFoundError);
  });

  it('InvalidArgumentError on missing/blank uid, by, toStatus, or citations[i].file', async () => {
    await expect(transition(store, { uid: '', by: 'closer', toStatus: 'open', note: 'x' })).rejects.toThrow(InvalidArgumentError);
    await expect(transition(store, { uid: issueUid, by: '  ', toStatus: 'open', note: 'x' })).rejects.toThrow(InvalidArgumentError);
    await expect(transition(store, { uid: issueUid, by: 'closer', toStatus: '', note: 'x' })).rejects.toThrow(InvalidArgumentError);
    await expect(
      transition(store, { uid: issueUid, by: 'closer', toStatus: 'open', note: 'x', citations: [{ file: '  ' }] }),
  ...
  it('an untyped caller sending an explicit `null` for uid/by/toStatus gets a clean InvalidArgumentError, never an unhandled TypeError', async () => {
    ...
    const nullUid = { uid: null, by: 'closer', toStatus: 'open', note: 'x' } as unknown as Parameters<typeof transition>[1];
    const nullBy = { uid: issueUid, by: null, toStatus: 'open', note: 'x' } as unknown as Parameters<typeof transition>[1];
    const nullToStatus = { uid: issueUid, by: 'closer', toStatus: null, note: 'x' } as unknown as Parameters<typeof transition>[1];
    await expect(transition(store, nullUid)).rejects.toThrow(InvalidArgumentError);
    await expect(transition(store, nullBy)).rejects.toThrow(InvalidArgumentError);
    await expect(transition(store, nullToStatus)).rejects.toThrow(InvalidArgumentError);
  });
  ...
  it('StaleSupersedeError against a uid a PRIOR `update` body-change already superseded — transition never mutates a retired identity', async () => {
    ...
    await expect(transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress', note: 'x' })).rejects.toThrow(StaleSupersedeError);
  });

  it('writes exactly one audit row (action:"transitioned", from/to = status names, note = input.note)', async () => {
    await transition(store, { uid: issueUid, by: 'closer', toStatus: 'in-progress', note: 'moving along' });
  ...
  it('a same-status transition (no-op status-wise) is STILL a real, fully-audited event — SPEC.md states no same-status exemption', async () => {
    const outcome = await transition(store, { uid: issueUid, by: 'closer', toStatus: 'open', note: 'reasserting open' });
    expect(outcome.fromStatus).toBe('open');
