...
async function countLiveOwnsComponentEdges(
  store: TestIssueStore,
  issueRowid: number
): Promise<number> {
...
describe('move — reparent an issue between projects/components (SPEC.md §6.3.6, §8 AC-17, real store)', () => {
  ...
  it('cross-project move with an explicit component: fromProject !== toProject, AC-17 still holds', async () => {
    const targetComponentB = await addComponent(
      store,
      projectBUid,
      'component-y'
    );
  ...
  it('same-placement call is a STATED no-op: noop:true, no edge invalidated/written, no audit row (teeth: audit count and live edge rowid unchanged)', async () => {
    ...
    const { rows: beforeEdge } = await store.adapter.executeAll<{
      rowid: number;
    }>(
    ...
    const { rows: afterEdge } = await store.adapter.executeAll<{
      rowid: number;
    }>(
      `SELECT rowid FROM edge WHERE dst = ? AND rel = 'owns_component' AND t_invalid IS NULL`,
      [issueRowid]
  ...
  it('IssueNotFoundError for a uid that does not resolve to a live issue', async () => {
    await expect(
      move(store, { uid: 'not-a-real-uid', by: 'agent-a' })
  ...
  it('IssueNotFoundError for a uid whose issue has already been soft-deleted (never re-movable)', async () => {
    ...
    await expect(move(store, { uid: issueUid, by: 'agent-a' })).rejects.toThrow(
      IssueNotFoundError
    );
  });

  it('InvalidArgumentError on missing/blank uid or by', async () => {
    await expect(move(store, { uid: '', by: 'agent-a' })).rejects.toThrow(
      InvalidArgumentError
    );
    await expect(move(store, { uid: issueUid, by: '   ' })).rejects.toThrow(
      InvalidArgumentError
    );
  });
});
