...
## `d8f0c34b` — errors.ts SQLITE\_\*-code helpers never match against the live Turso driver — code is always GenericFailure
...
**Why this is worth an item rather than four `eslint-disable`s.** This rule was written _for_ this
...
are _guarded_, not unguarded, so they are not live bugs today. That is precisely why they are still
