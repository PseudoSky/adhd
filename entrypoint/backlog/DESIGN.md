...
| Live HTTP mount            | `src/server.e2e.ts`                                                                                     | `startBacklogServer({transport:'http', signal})` against a real temp store file, then a real `fetch()` call — resource-consuming (binds a real port), so it now runs under the separately-invoked e2e suite per `AGENTS.md`'s "Live testing is mandatory".                  |
