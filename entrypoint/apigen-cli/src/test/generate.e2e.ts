...
describe('generate command', () => {
  ...
  it('resolves --export myApi to ExportMode { type: named-object, name: myApi }', () => {
    expect(resolveExportMode('myApi')).toEqual({
      type: 'named-object',
      name: 'myApi',
    });
  });
...
describe('generate-registry command', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'apigen-cli-registry-test-')
    );
  });
  ...
  it('unions deps from multiple operations', () => {
    const schemas = {
      opA: {
        input: {
          type: 'object',
          properties: { p: { type: 'string', format: 'decimal' } },
        },
        output: { type: 'null' },
      },
      opB: {
...
describe('dep-manifest: patchPackageJsonDeps (integration — file I/O)', () => {
  ...
  it('writes decimal.js into package.json when schemas carry format:decimal', () => {
    ...
    const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8')) as Record<
      ...
      unknown
    >;
    const writtenDeps = pkg['dependencies'] as
      | Record<string, string>
      | undefined;

    expect(writtenDeps, 'package.json should have dependencies').toBeDefined();
    expect(writtenDeps?.['decimal.js'], 'decimal.js must be pinned').toBe(
      '^10'
    );
  ...
  it('[teeth] patchPackageJsonDeps is a no-op when dep map is empty — proves collection step is load-bearing', () => {
    ...
    expect(
      deps['decimal.js'],
      'decimal.js must NOT appear when dep map is empty'
    ).toBeUndefined();
  });
});
...
describe('dep-manifest: generate command — negative control', () => {
  ...
  it('[negative-control] does NOT declare decimal.js for a surface with no Decimal types', async () => {
    await runGenerate([
      'generate',
      '--source',
      apiFixture, // api.ts: plain string/void params — no decimal
      '--type',
      'jsonschema',
      '--out-dir',
      tmpDir,
    ...
    expect(
      deps['decimal.js'],
      'decimal.js must NOT appear for a non-Decimal surface'
    ).toBeUndefined();
  });
});
...
describe('[dod.10 teeth] decimal default-import: schema carries format:decimal and package.json declares decimal.js', () => {
  ...
  it('v2 generate pipeline: generated package.json declares decimal.js for a default-imported Decimal source', async () => {
    ...
    if (addAmountsSchema) {
      ...
      for (const f of collectFormats(addAmountsSchema.output))
        allFormats.add(f);
    }
    ...
    expect(
      ...
      `format:decimal not found in composed schema — schema = ${JSON.stringify(
        addAmountsSchema
      )}. ` +
    ...
    expect(
      deps['decimal.js'],
      `decimal.js not in deps — deps=${JSON.stringify(
        deps
      )}. format:decimal was lost before collectLogicalTypeDeps.`
    ).toBe('^10');
  }, 30_000);
  ...
  it('full CLI generate: package.json in output declares decimal.js for a default-imported Decimal source', async () => {
    ...
    expect(
      ...
      'package.json must be emitted by the generate command'
    ).toBe(true);
    ...
    expect(
      deps['decimal.js'],
      `generated package.json does not declare decimal.js (deps=${JSON.stringify(
        deps
      )}). ` +
  ...
  it('[negative-control] generated package.json does NOT declare decimal.js when patching is bypassed', () => {
