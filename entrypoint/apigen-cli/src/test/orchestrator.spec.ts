...
describe('orchestrator: detect → extract → merge', () => {
  ...
  it('mergeOperations flattens two per-source arrays into a single list', () => {
    const a: Operation[] = [
      makeOp('ns-a/file-a/get-user', 'ns-a', 'file-a', 'getUser'),
    ];
    const b: Operation[] = [
      makeOp('ns-b/file-b/send-email', 'ns-b', 'file-b', 'sendEmail'),
    ];
  ...
  it('buildDescriptor extracts operations from two sources and merges into one descriptor', async () => {
  ...
  it('orchestrateGenerate calls plugin.generate with packages from both sources', async () => {
...
describe('orchestrator: projection-override config (Tenet 1)', () => {
  ...
  it('buildDescriptor passes the override config through to collision check without error', async () => {
...
describe('[dod.10 v2 teeth] buildDescriptor: default-import Decimal source carries format:decimal in packageSchemas', () => {
  ...
  it('buildDescriptor packageSchemas carry format:decimal for a default-imported Decimal source', async () => {
    const descriptor = await buildDescriptor({
    ...
    expect(
      addAmountsSchema,
      'addAmounts must be present in packageSchemas'
    ).toBeDefined();
    ...
    expect(
      ...
      `format:decimal not in v2 packageSchemas schema — schema=${JSON.stringify(
        addAmountsSchema
      )}. ` +
