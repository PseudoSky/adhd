  ...
  const adapter: HostAdapter = {
    ...
    async invoke(request, _signal) {
      if (!proc || processExited)
        throw new Error(`python host ${host} not serving`);
      return rpc('invoke', {
...
describe('gateway mixed-host: REAL TS + REAL Python sidecar', () => {
  it('routes each op to its owning runtime; both return ground truth (dod.12)', async () => {
    ...
    const tsResult = (await gateway.route(
      req(op('ts', 'ts/double'), { n: 21 })
    )) as {
      host: string;
      value: number;
    };
  ...
  it('killing the Python sidecar fails ONLY its ops; TS keeps serving (dod.17)', async () => {
    ...
    const tsBefore = (await gateway.route(
      req(op('ts', 'ts/double'), { n: 5 })
    )) as { value: number };
    ...
    const tsAfter = (await gateway.route(
      req(op('ts', 'ts/double'), { n: 9 })
    )) as { value: number };
