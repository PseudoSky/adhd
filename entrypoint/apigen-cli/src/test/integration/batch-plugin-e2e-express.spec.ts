 ...
 * sibling `batch-plugin-e2e-express.e2e.ts`.
 *
 * Resource lane: proc — it starts a real in-process Express HTTP server bound to a port.
 *
 ...
 * `batch-plugin-e2e-express.e2e.ts`); they are the contract a mocked version must satisfy
 * without touching a subprocess, a port, or the real extraction pipeline.
 */
import { describe, it } from 'vitest';

describe('mocked: batch-plugin-e2e-express', () => {
