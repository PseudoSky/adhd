 ...
 * actually works over a REAL express server (BATCH_0.0.1.md §2/§F1 —
 * batch-rollout design note + architect review), not just that the pieces
 * compile.
 *
 ...
 * never actually been driven over real HTTP.
 *
 * REAL components, per CLAUDE.md §7 — no bypass, no mocks of the thing under
 * test:
 *   - a REAL `@adhd/apigen-plugin-api-express` server (`run()`), started on an
 ...
 *     `buildBatchMountedOperations` + the REAL hostBridge express's `run.ts`
 *     constructs (merged, package-spanning `schemasByOpId`/`fnsByOpId`) —
 *     proving the mount-collection hoist and the `MountHostBridge` wiring
 *     both work together, end to end, on the express adapter.
 *
...
import { run as runExpress } from '@adhd/apigen-plugin-api-express';
...
describe('[BATCH_0.0.1.md §2/§F1] apigen-plugin-batch — real e2e over a live express server', () => {
  it('POST /_batch/action fans out to a REAL registered operation and returns real, ordered results (partial-failure + concurrency respected)', async () => {
    ...
    serverPromise = runExpress(runInput);
