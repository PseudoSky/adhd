    ...
    for (const pkg of packages) {
      ...
      for (const [key, fn] of Object.entries(pkg.fns ?? {})) {
        expect(typeof fn).toBe(
          'function',
          `expected fns.${key} to be a function`
        );
      }
...
describe('[dod.fail-fast.a] assertFnsNonEmpty — 0-function source throws actionable message', () => {
  ...
  it('error message contains "0 functions found"', () => {
    expect(() => assertFnsNonEmpty(emptyFns, fakeSource)).toThrowError(
      /0 functions found/
    );
  });
  ...
  it('error message contains actionable hint about wrong source file', () => {
    expect(() => assertFnsNonEmpty(emptyFns, fakeSource)).toThrowError(
      /generated output or the wrong source file/
    );
  });
...
describe('[dod.fail-fast.b] assertDecimalLibPresent — absent decimal.js errors at startup', () => {
  ...
  const presentResolver = (_pkg: string): string =>
    '/node_modules/decimal.js/index.js';
  ...
  it('throws when a decimal function is present and the lib is absent', () => {
    expect(() =>
      assertDecimalLibPresent(decimalSchemas, absentResolver)
    ).toThrow();
  });
  ...
  it('does NOT throw when decimal.js is present (resolver succeeds)', () => {
    expect(() =>
      assertDecimalLibPresent(decimalSchemas, presentResolver)
    ).not.toThrow();
  });
  ...
  it('does NOT throw when no function uses decimal format (lib absence is irrelevant)', () => {
    ...
    expect(() =>
      assertDecimalLibPresent(noDecimalSchemas, absentResolver)
    ).not.toThrow();
  });
});
...
describe('[cli-run-cmd.non-ts] non-TS plugin bypasses TS extraction', () => {
  it(
    'calls plugin.run() with importPath set to the source file, no TS compilation',
    { timeout: 10000 },
    async () => {
      let capturedInput: RunInput | undefined;
  ...
  it(
    ...
    async () => {
      ...
      await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(
            () => reject(new Error('server did not shut down within 2 s')),
            2000
          )
        ),
      ]);
