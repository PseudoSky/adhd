  ...
  for (let f = 0; f < FILES; f++) {
    ...
    for (let i = 0; i < FNS; i++) {
      switch (i % 3) {
        case 0:
          lines.push(
            `export function fn${i}_named(item: IItem${f}, n: number): IItem${f}[] { return [item] }`
          );
          break;
        case 1:
          lines.push(
            `export async function fn${i}_inline(input: { when: Date; tags: string[] }): Promise<{ ok: boolean }> { return { ok: !!input } }`
          );
          break;
        case 2:
          lines.push(
            `export const fn${i}_arrow = (m: Map<string, Date>, mode: 'a' | 'b'): [Date, string] => [new Date(), mode]`
          );
          break;
      }
    }
...
describe('[perf] buildDescriptor through the real orchestrator', () => {
  ...
  it('heap stays flat across repeated runs (no per-run Project/generator leak)', async () => {
    ...
    if (typeof gc !== 'function') {
      ...
      throw new Error(
        'global.gc missing — vite.config test.poolOptions must pass --expose-gc'
      );
    }
