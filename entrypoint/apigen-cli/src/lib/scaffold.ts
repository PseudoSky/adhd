...
function resolveDepVersions(
  ...
  const adhdSources = workspaceRoot
    ? workspaceAdhdSources(workspaceRoot)
  ...
  for (const name of names) {
    ...
    try {
      if (name.startsWith('@adhd/')) {
        ...
        const pkgJson =
          srcDir && path.join(path.dirname(srcDir), 'package.json');
 ...
 */
function absoluteWorkspacePaths(
  ...
  for (const entry of fs.readdirSync(workspaceNm, { withFileTypes: true })) {
    ...
    if (name.startsWith('@')) {
      ...
      for (const pkg of fs.readdirSync(scopeSrc, { withFileTypes: true })) {
        trySymlink(
          path.join(scopeSrc, pkg.name),
          path.join(scopeDest, pkg.name)
        );
      }
    } else {
...
export function emitResolutionScaffolding(
  ...
  const workspaceRoot =
    findWorkspaceRoot(outputDir) ?? findWorkspaceRoot(__dirname);
  const deps = resolveDepVersions(
    workspaceRoot,
    PLUGIN_RUNTIME_DEP_NAMES[pluginId] ?? []
  );
  ...
  const paths =
    opts.linkWorkspace && workspaceRoot
      ? absoluteWorkspacePaths(workspaceRoot)
      : {};
  if (
    writeIfAbsent(
      tsconfigPath,
  ...
  if (
    opts.linkWorkspace &&
    workspaceRoot &&
    linkNodeModules(outputDir, workspaceRoot)
  ) {
