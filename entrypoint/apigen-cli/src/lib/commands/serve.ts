...
export function parseMounts(pairs: string[]): Record<string, string> {
  ...
  for (const pair of pairs) {
    ...
    if (i <= 0 || i === pair.length - 1) {
      throw new Error(`--mount expects <namespace>=<plugin> (got "${pair}")`);
    }
...
export function resolveHosts(
  ...
  for (const src of sources) {
    ...
    const transport: 'http' | 'grpc' = GRPC_PLUGINS.has(plugin)
      ? 'grpc'
      : 'http';
    hosts.push({
      namespace,
 ...
 */
const GRPC_TRAILER_KEYS = new Set([
  'grpc-status',
  'grpc-message',
  'grpc-encoding',
]);
...
function proxyGrpcStream(
  ...
  const fwdHeaders: http2.OutgoingHttpHeaders = {
  ...
  stream.on('data', (chunk: Buffer) => {
    if (!outStream.destroyed) outStream.write(chunk);
  });
  stream.on('end', () => {
  ...
  outStream.once(
    'response',
    (respHeaders: http2.IncomingHttpHeaders, flags: number) => {
      if (stream.destroyed || stream.closed) return;
      ...
      if (clientHeaders[':status'] === undefined)
        clientHeaders[':status'] = 200;
      ...
      if (endStreamInHeaders) {
        ...
        pendingTrailers =
          Object.keys(trailersFromHeaders).length > 0
            ? trailersFromHeaders
            : { 'grpc-status': '0' };
        try {
  ...
  outStream.once('error', (err: Error) => {
    if (!stream.destroyed)
      sendGrpcUnavailable(stream, ns, `upstream error: ${err}`);
  });
...
function handleHttp1Request(
  ...
  if (ns === '_meta') {
    ...
    res.end(
      JSON.stringify({
        code: 'not_found',
        message: `unknown meta route ${pathOnly}`,
      })
    );
    return;
  }
  ...
  if (ns === 'grpc') {
    ...
    if (!reflHost) {
      sendGrpcUnavailable(
        stream,
        'grpc',
        'no gRPC host available for reflection'
      );
      return;
    }
...
export function createFrontServer(hosts: Host[]): http.Server {
  ...
  rawServer.on('connection', (socket: net.Socket) => {
    ...
    socket.once('readable', () => {
      ...
      const isH2 =
        peeked.length >= 3 && peeked.slice(0, 3).equals(H2_PREFACE.slice(0, 3));
  ...
  (httpServer as http.Server & { _rawServer?: net.Server })._rawServer =
    rawServer;
...
export async function startServe(opts: {
  ...
  const userOverrode =
    pyOverride !== undefined && pyOverride !== _managedPython;
  ...
  for (const h of hosts) {
    ...
    log(
      `[serve] spawned "${h.namespace}" (${h.plugin}/${h.transport}) → :${h.port}  [${h.source}]`
    );
  }
  ...
  for (const h of hosts) {
    log(
      `[serve]   /${h.namespace}/* → :${h.port} (${h.plugin}/${h.transport})`
    );
  }
...
export function registerServeCommand(program: Command): void {
  program
    ...
    .requiredOption('--port <port>', 'Front HTTP/gRPC port', (v) =>
