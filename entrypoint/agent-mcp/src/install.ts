/**
 * install.ts — `agent-mcp-install` (BUG-AGENTMCP-006). Registers the
 * `agent-mcp` MCP server into the selected agent host's config file at the
 * selected scope, so a consumer can onboard the server with zero manual
 * config editing — no hand-written `.mcp.json`/`opencode.json`/
 * `~/.claude.json`/`config.toml` entries required.
 *
 * This module is modeled directly on `entrypoint/backlog/src/install.ts`
 * (the repo's established, working `backlog install` MCP-registration
 * surface): same host/scope grammar, same per-host config-path resolution
 * table, same idempotent JSON upsert and line-oriented TOML upsert, same
 * `homeOverride` test-isolation escape hatch. The only substantive
 * difference is the target server: this file registers `agent-mcp` (bin
 * `agent-mcp-install`) instead of `backlog`, and it has no skill half — a
 * pure MCP-registration command.
 *
 * No new external dependency: the two MUST-HAVE hosts (Claude Code,
 * OpenCode) both use plain JSON, handled with `JSON.parse`/
 * `JSON.stringify` — nothing to add. Codex uses TOML; no `toml` package is a
 * declared dependency of this package, so — exactly like `backlog
 * install` — the narrow, fully self-contained `upsertTomlTable` below
 * (replace-or-append exactly ONE known table, `[mcp_servers.agent-mcp]`,
 * whose own contents this file fully controls) is the safe,
 * zero-dependency choice.
 */
import { existsSync, mkdirSync, readFileSync, writeFileSync } from 'node:fs';
import { homedir } from 'node:os';
import { dirname, join } from 'node:path';

export type McpHost = 'claude' | 'codex' | 'opencode';
export type McpScope = 'user' | 'project';

export const ALL_HOSTS: readonly McpHost[] = ['claude', 'codex', 'opencode'];

/**
 * The one, portable invocation every host's registration entry points at —
 * `npx -y @adhd/agent-mcp@latest` — so a written config works on any machine
 * with `npx` and network access, never tied to this monorepo's own local
 * `dist/src/index.js` path (that local-path form is what THIS repo's own
 * checked-in `.mcp.json` uses for its own dev loop, which is deliberately
 * NOT what a published `install` command should write for a third-party
 * consumer). Same design as `backlog install`'s
 * `BACKLOG_MCP_NPX_ARGS` (`backlog/src/install.ts:54`).
 */
export const AGENT_MCP_NPX_ARGS: readonly string[] = ['-y', '@adhd/agent-mcp@latest'];

export type McpInstallStatus = 'written' | 'manual';

export interface McpInstallResult {
  host: McpHost;
  scope: McpScope;
  configPath: string;
  status: McpInstallStatus;
  note?: string;
}

function readJsonFile(path: string): Record<string, unknown> {
  if (!existsSync(path)) return {};
  const raw = readFileSync(path, 'utf8').trim();
  if (raw === '') return {};
  const parsed: unknown = JSON.parse(raw);
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(
      `agent-mcp-install: expected ${path} to contain a JSON object, got ${Array.isArray(parsed) ? 'an array' : typeof parsed}`,
    );
  }
  return parsed as Record<string, unknown>;
}

/**
 * Deep-merges ONE entry (`entryKey` -> `entryValue`) into the object living
 * at `containerKey` inside the JSON file at `path`, preserving every other
 * top-level key in the file AND every other entry already inside the
 * container object. Creates the file (and the container object) if either
 * is absent. Re-serializes with a stable 2-space indent (matching every
 * hand-authored config this repo already ships — `.mcp.json`,
 * `opencode.json`, `~/.claude.json`) and a trailing newline. Mirrors
 * `backlog/src/install.ts:86-97`.
 */
function upsertJsonMcpEntry(path: string, containerKey: string, entryKey: string, entryValue: unknown): void {
  const doc = readJsonFile(path);
  const containerRaw = doc[containerKey];
  const container: Record<string, unknown> =
    typeof containerRaw === 'object' && containerRaw !== null && !Array.isArray(containerRaw)
      ? (containerRaw as Record<string, unknown>)
      : {};
  container[entryKey] = entryValue;
  doc[containerKey] = container;
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, JSON.stringify(doc, null, 2) + '\n', 'utf8');
}

function claudeConfigPath(scope: McpScope, cwd: string, homeOverride?: string): string {
  if (scope === 'project') return join(cwd, '.mcp.json');
  const home = homeOverride ?? homedir();
  return join(home, '.claude.json');
}

function opencodeConfigPath(scope: McpScope, cwd: string, homeOverride?: string): string {
  if (scope === 'project') return join(cwd, 'opencode.json');
  const home = homeOverride ?? homedir();
  return join(home, '.config', 'opencode', 'opencode.json');
}

function codexConfigPath(scope: McpScope, cwd: string, homeOverride?: string): string {
  if (scope === 'project') return join(cwd, '.codex', 'config.toml');
  const home = homeOverride ?? homedir();
  return join(resolveCodexHomeDir(home, homeOverride), 'config.toml');
}

/** Same `CODEX_HOME` resolution rule as `backlog/src/install-skill.ts:45-49`:
 *  honored only for the REAL machine home, never under a test's isolated
 *  `homeOverride` (which must be fully self-contained). */
function resolveCodexHomeDir(home: string, homeOverride?: string): string {
  return homeOverride ? join(home, '.codex') : (process.env['CODEX_HOME'] ?? join(home, '.codex'));
}

/**
 * Replaces (or appends) exactly one `[tableHeader]` table in a TOML
 * document's text, leaving every other line — including every OTHER
 * table — byte-for-byte untouched. Scoped deliberately narrow (see this
 * file's header doc): it finds the line matching `[tableHeader]` exactly,
 * then treats everything up to (but not including) the next line that
 * starts a new table (`[...`) — or end of file — as "this table's body",
 * and swaps that whole span for `newBodyLines`. If the table doesn't exist
 * yet, the new table is appended at the end (separated by a single blank
 * line from any existing content).
 *
 * This is intentionally NOT a general TOML parser/editor — it only
 * recognizes bracketed table headers on their own line, which is exactly
 * (and only) the shape this file ever writes or looks for. Mirrors
 * `backlog/src/install.ts:132-156`.
 */
export function upsertTomlTable(text: string, tableHeader: string, newBodyLines: readonly string[]): string {
  const headerLine = `[${tableHeader}]`;
  const lines = text.length > 0 ? text.split('\n') : [];
  const headerIndex = lines.findIndex((line) => line.trim() === headerLine);
  const newBlock = [headerLine, ...newBodyLines];

  if (headerIndex === -1) {
    if (lines.length === 0 || lines.every((line) => line.trim() === '')) {
      return newBlock.join('\n') + '\n';
    }
    // Trim any trailing blank lines before appending, then separate with
    // exactly one blank line — keeps re-running this idempotent (never
    // accumulates extra blank lines on repeat appends elsewhere).
    while (lines.length > 0 && lines[lines.length - 1]?.trim() === '') lines.pop();
    return [...lines, '', ...newBlock].join('\n') + '\n';
  }

  let endIndex = headerIndex + 1;
  while (endIndex < lines.length && !(lines[endIndex]?.trim().startsWith('[') ?? true)) endIndex++;
  const result = [...lines.slice(0, headerIndex), ...newBlock, ...lines.slice(endIndex)];
  // Normalize a possible trailing empty entry produced by a trailing
  // newline in the original file, then always end with exactly one.
  while (result.length > 0 && result[result.length - 1] === '') result.pop();
  return result.join('\n') + '\n';
}

function tomlStringArray(values: readonly string[]): string {
  return `[${values.map((v) => JSON.stringify(v)).join(', ')}]`;
}

function registerMcpClaude(scope: McpScope, cwd: string, homeOverride?: string): McpInstallResult {
  const configPath = claudeConfigPath(scope, cwd, homeOverride);
  upsertJsonMcpEntry(configPath, 'mcpServers', 'agent-mcp', {
    type: 'stdio',
    command: 'npx',
    args: [...AGENT_MCP_NPX_ARGS],
  });
  return { host: 'claude', scope, configPath, status: 'written' };
}

function registerMcpOpencode(scope: McpScope, cwd: string, homeOverride?: string): McpInstallResult {
  const configPath = opencodeConfigPath(scope, cwd, homeOverride);
  upsertJsonMcpEntry(configPath, 'mcp', 'agent-mcp', {
    type: 'local',
    command: ['npx', ...AGENT_MCP_NPX_ARGS],
  });
  return { host: 'opencode', scope, configPath, status: 'written' };
}

function registerMcpCodex(scope: McpScope, cwd: string, homeOverride?: string): McpInstallResult {
  const configPath = codexConfigPath(scope, cwd, homeOverride);
  const existing = existsSync(configPath) ? readFileSync(configPath, 'utf8') : '';
  const updated = upsertTomlTable(existing, 'mcp_servers.agent-mcp', [
    'command = "npx"',
    `args = ${tomlStringArray(AGENT_MCP_NPX_ARGS)}`,
  ]);
  mkdirSync(dirname(configPath), { recursive: true });
  writeFileSync(configPath, updated, 'utf8');
  return { host: 'codex', scope, configPath, status: 'written' };
}

function registerMcp(host: McpHost, scope: McpScope, cwd: string, homeOverride?: string): McpInstallResult {
  if (host === 'claude') return registerMcpClaude(scope, cwd, homeOverride);
  if (host === 'opencode') return registerMcpOpencode(scope, cwd, homeOverride);
  return registerMcpCodex(scope, cwd, homeOverride);
}

interface ParsedInstallArgs {
  hosts: McpHost[];
  scope: McpScope;
}

function parseInstallArgs(argv: string[]): ParsedInstallArgs {
  let hostArg = 'all';
  let scope: McpScope = 'user';
  for (let i = 0; i < argv.length; i++) {
    const arg = argv[i];
    if (arg === '--host') {
      // DEBT-AGENTMCP-INSTALL-HOST-DEFAULT-001: a bare trailing --host (no
      // value after it) must error — never silently degrade to the "all"
      // default and install into every host's config. `all` is only legal
      // when spelled out literally.
      const value = argv[i + 1];
      if (value === undefined) {
        throw new Error(
          `agent-mcp-install: --host requires a value (${[...ALL_HOSTS, 'all'].join('|')}) — got a bare trailing --host with no value; see --help for usage`,
        );
      }
      i++;
      hostArg = value;
    } else if (arg === '--scope') scope = (argv[++i] as McpScope) ?? 'user';
    else throw new Error(`agent-mcp-install: unknown argument "${arg}" (expected --host/--scope)`);
  }
  if (scope !== 'user' && scope !== 'project') {
    throw new Error(`agent-mcp-install: --scope must be "user" or "project", got "${scope}"`);
  }
  const hosts = hostArg === 'all' ? [...ALL_HOSTS] : [hostArg as McpHost];
  for (const h of hosts) {
    if (!ALL_HOSTS.includes(h)) {
      throw new Error(`agent-mcp-install: --host must be one of ${ALL_HOSTS.join('|')}|all, got "${hostArg}"`);
    }
  }
  return { hosts, scope };
}

export const INSTALL_HELP_TEXT = `agent-mcp-install [--host claude|opencode|codex|all] [--scope user|project]

Registers the agent-mcp MCP server into an agent host's config so an agent
host can connect to it — no manual config editing required.

  --host <name>   claude | opencode | codex | all (default: all)
  --scope <name>  user | project (default: user)

Examples:
  agent-mcp-install --host opencode --scope user
  agent-mcp-install --host claude --scope project
`;

export interface InstallResult {
  mcp: McpInstallResult[];
}

/**
 * Registers the `agent-mcp` MCP server into the requested host config(s),
 * per `--host`/`--scope`. Idempotent and non-destructive: re-running
 * overwrites only the `agent-mcp` MCP entry, never touching anything else
 * already present in the config file.
 *
 * `homeOverride` is a TEST-ISOLATION ESCAPE HATCH ONLY, mirroring
 * `backlog/src/install.ts:269`'s parameter — never passed by
 * `runInstallCommand`/the real CLI.
 */
export function install(argv: string[], cwd: string = process.cwd(), homeOverride?: string): InstallResult {
  const { hosts, scope } = parseInstallArgs(argv);
  const mcp = hosts.map((host) => registerMcp(host, scope, cwd, homeOverride));
  return { mcp };
}

/** CLI entry for `agent-mcp-install` — same `console.log(JSON.stringify(...))`
 *  output convention as `backlog install`'s `runInstallCommand`
 *  (`backlog/src/install.ts:281-294`), plus a short human-readable summary
 *  of what was written where, printed to stderr so it never disturbs the
 *  machine-readable stdout JSON a script might parse. */
export async function runInstallCommand(argv: string[]): Promise<void> {
  if (argv[0] === '--help' || argv[0] === '-h') {
    console.log(INSTALL_HELP_TEXT);
    return;
  }
  const result = install(argv);
  for (const m of result.mcp) {
    const suffix = m.note ? ` — ${m.note}` : '';
    console.error(`[agent-mcp-install] mcp   -> ${m.host} (${m.scope}) [${m.status}]: ${m.configPath}${suffix}`);
  }
  console.log(JSON.stringify(result));
}
