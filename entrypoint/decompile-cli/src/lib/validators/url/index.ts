...
const normalizeDataURL = (
  ...
  const header = `data:${normalizedMediaType.join(';')}`;
  const data = `${base64 ? body.trim() : body}${hash ? `#${hash}` : ''}`;
  return `${header},${data}`;
};
...
export const defaultOptions: NormalizeOptions = {
  ...
  stripHash: false,
  stripWWW: true,
  returns: 'string', // "string" -> "" or "url" -> <URL>
  removeQuery: false,
...
export const normalizeUrl = (
  ...
  if (/^data:/i.test(urlString)) {
    return normalizeDataURL(urlString, { stripHash: false, ...options });
  }
...
export default { ensure: normalizeUrl, isValidUrl, getPatch };
