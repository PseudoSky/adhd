...
| 0b | `Discover` | *(only when `priorArt` is omitted)* queries the backlog graph for open items relevant to the swept scope | prior-art set |
...
## Prior art: the sweep dedupes itself

A sweep that cannot see what is already filed re-derives it. On 2026-08-12 a run produced five epics
that duplicated an earlier sweep's, at roughly 3.7M tokens. The synthesiser has always accepted
`priorArt` — but assembling it was the *caller's* job, and a caller who forgot got no warning, just a
full set of epics tagged `UNKNOWN` that read exactly like new discoveries.

Stage 0b closes that hole. When `priorArt` is omitted, a cheap agent queries the backlog graph
directly, keeps the open items whose subsystem/package/symbol overlaps the swept units, and hands
them to the synthesiser. Caller-supplied `priorArt` still wins — same principle as `nxProjects`:
deterministic data the caller computed beats an agent transcribing it.

Three properties worth knowing:

- **It never fails the sweep.** No backlog CLI, no graph, a query that errors — each degrades to an
  empty prior-art set with a loud `PRIOR ART: UNAVAILABLE` log line. A sweep without dedupe is still
  worth having; a sweep that refuses to run is not.
- **It is repo-scoped, deliberately.** The graph is shared across repositories. Unpinned, the agent
  infers the repo from the checkout and reports what it kept in `reposUsed`; pass `priorArtRepos` to
  pin it. A sibling repo's items are not prior art for this sweep.
- **It biases toward keeping.** A false keep costs a few tokens of synthesiser context. A false drop
  silently recreates a ticket that already exists.

`metrics.duplicates.priorArtSource` reports which path ran: `caller`, `discovered`, or `skipped`.
...
| `priorArt` | *(auto-discovered)* | `[{id, title}]` of already-filed items. Each epic comes back tagged `NEW` / `CORROBORATES <id>` / `EXTENDS <id>`. Omit to let Stage 0b query the backlog itself; pass explicitly to override |
| `priorArtRepos` | *(inferred)* | Repo name(s) to keep prior art from. The backlog graph is shared across repos — pin this when the inferred name might drag in a sibling repo's items |
| `skipPriorArtDiscovery` | `false` | Skip the prior-art query and sweep blind. For a repo with no backlog, or when you have already reconciled externally |
