...
2. **Stage 2 — ephemeral merge + scoped verification**, only if stage 1 passed:
   ...
   - **Narrower, opt-in fast path:** the workspace also ships `pnpm test:changed` (dirty tree) and `pnpm test:related -- <files…>` — a workspace-wide `vitest related` selection that **exits non-zero with `no tests selected`** rather than reporting success on an empty selection. These are explicit opt-ins; the full `nx affected -t test` path above remains the default. Semantics and measured behaviour: `docs/plan/nx-23-upgrade/TEST-SELECTION.md`.
