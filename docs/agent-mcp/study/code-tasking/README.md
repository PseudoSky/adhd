...
| | |
|---|---|
| Models compared | `gemma-4-e4b`, `qwen2.5-14b-instruct`, `qwen3.5-9b-claude-4.6-highiq-…`, `qwen3-coder-30b-a3b` (LM Studio) · `claude-haiku-4-5`, `claude-sonnet-4-6` (Anthropic) — same prompts, provider swapped |
...
9. **A capability ladder, and the rung that matters is diagnosis** (Experiments 7–9). Four models, identical prompts, by-requirement strict pass-rate — floor is flat, diagnosis fans out:
   ...
   |---|---|---|---|---|---|---|
   | `DIAGNOSE` strict PASS | 0/9 | 0/9 | 1/9 | 1/9 | 4/9 | **9/9** |
   ...
   - **Code-specialization ≠ diagnosis** — the 30B coder (Qwen3-Coder-30B) is the first *local* model to catch the audit comment-trap and clears APPLY 4/4, but its cold FK diagnosis (1/9) ties the 9B distill. It buys careful code-reading + application, not cross-layer synthesis.
