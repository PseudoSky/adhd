...
## Experiment 11 — code-specialization at scale: Qwen3-Coder-30B-A3B (full battery, **temp=0**)
> Does a dedicated 30B *coder* (MoE, 3B active; MLX-4bit on the M1 Max) crack the FK diagnosis wall that general-instruct and the distill couldn't? Ran all 18 on `qwen3-coder-30b-a3b-instruct-mlx`, at **temperature 0 (greedy)** / max_tokens 8192 — the **only model run with controlled sampling** (the other five ran at provider-default temp). Responses in `results/runs.qwen3-coder-30b.jsonl`. (First load failed — LM Studio's loading guardrail blocked the 256k-context default; relaxed + a one-shot probe, then ran.)

**Result: 8 PASS / 3 NEAR / 7 FAIL (score 9.5/18, 53%) — slots between qwen3.5-9b and haiku.** Code-specialization wins: **first *local* model to catch the audit comment-trap (T8** — fix skips `//` lines), and **APPLY 3/4** (T5 the cleanest connection-level fix of any small model). On SSE (T1/T7) it adds the right `server.on('error')` handler but mis-frames the mechanism as "throws an unhandled exception" → `NEAR`. **Cold FK `DIAGNOSE` is 2/9** — T13 (greedy) correctly nailed it ("Drizzle's transaction wrapper doesn't respect the in-script `PRAGMA OFF`; set FK off before migrate" + rejected the planted wrong dx), and T8; but T2/T3/T6/T9/T12 still confabulate (T9 even *denies* cascade), and T14 orchestrated correctly but with a wrong fix.
