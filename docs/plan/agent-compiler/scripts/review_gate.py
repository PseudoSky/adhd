#!/usr/bin/env python3
"""
review_gate.py — code-review gate for the agent-compiler plan.
...
no UNRESOLVED blocking findings. A diff-reading reviewer (architect-reviewer, opus)
...
audit_*.py oracle cannot (e.g. a cross-package join that violates the decided single-DB topology, a composition-order resolution that diverges from the recorded precedence contract, or a per-platform header builder that does not match the decided contract).

Usage:
  python3 docs/plan/agent-compiler/scripts/review_gate.py
