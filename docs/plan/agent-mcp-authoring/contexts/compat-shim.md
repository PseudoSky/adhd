...
**Phase:** compat · **Kind:** work · **Depends on:** agent-define · **Guard:** `npx --yes nx test agent-mcp --testFile=entrypoint/agent-mcp/src/__tests__/systemprompt-compat.test.ts`
...
```text
read_only:  []
mutates:    ["entrypoint/agent-mcp/src/tools/agent-crud.ts", "entrypoint/agent-mcp/src/validation/agent.ts", "entrypoint/agent-mcp/src/tools/guide.ts", "entrypoint/agent-mcp/src/__tests__/systemprompt-compat.test.ts"]
```
