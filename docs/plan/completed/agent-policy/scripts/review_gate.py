#!/usr/bin/env python3
"""
review_gate.py — code-review gate for the agent-policy plan.
...
no UNRESOLVED blocking findings. A diff-reading reviewer (code-reviewer, opus)
authors `review.md` against this project's CLAUDE.md standards and the plan's own
decisions.md/contracts — catching design-intent violations that the structural
audit_*.py oracle cannot (e.g. a composite key implemented as a non-unique `index()` instead of `primaryKey()`/`uniqueIndex()`, policy_types implemented as a SQL enum rather than a lookup table, an inheritance FK that does not match the decided topology, or a cross-package FK violation).

Usage:
  python3 docs/plan/agent-policy/scripts/review_gate.py
