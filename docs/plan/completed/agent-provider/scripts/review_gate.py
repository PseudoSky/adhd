#!/usr/bin/env python3
"""
review_gate.py — code-review gate for the agent-provider plan.
...
audit_*.py oracle cannot (e.g. an FK that does not match the decided provider/model/binding topology, a ProviderAdapter re-declared in agent-provider instead of agent-mcp-types, or a cross-package FK violation).

Usage:
  python3 docs/plan/agent-provider/scripts/review_gate.py
