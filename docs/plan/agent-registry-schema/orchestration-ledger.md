...
- **Recovery (needs human/session):** run `/sox-tools:reflection` in a session where the `memory-server` write tools are exposed, or reconnect the MCP surface — then write the 11 verbatim and `memory_link #11 DERIVED_FROM #9`.
