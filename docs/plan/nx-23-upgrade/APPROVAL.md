...
### Four audit-gate defects found by *running* the gates, and fixed

`gap-check` and `env-pin-check` were green both before and after; neither executes the audit
hold points. Running them exposed four plan-local defects that made every audit gate
**red by construction** and, once fixed, non-terminating:

1. `guard_audit_*.py` invoked the runner with `cwd=REPO_ROOT`, but the runner resolves
   `criteria.json` relative to `process.cwd()` → `[audit.no-criteria] FAIL` always.
   Fixed with an explicit `--criteria <abs>`.
2. The guards passed a comma-joined phase list (`--phase "intake,reconcile"`), which the
   runner rejects — it takes **one** phase and accumulates prior ones. Fixed by passing the
   terminal phase only.
3. Five audit criteria ran their own guard script, which re-ran the audit → unbounded
   recursion (masked by defects 1–2). Replaced with non-recursive `present` checks that pin
   the `--criteria` wiring.
4. `final-review.md` claimed `state-machine.md` was generated by a render script. No such
   renderer exists. Claim corrected.

Full detail in `final-review.md` § *Four audit-gate defects*. These are plan-local
(`docs/plan/nx-23-upgrade/scripts/`), not skill-level; the missing renderer in defect 4 is a
skill-level gap and is reported to the operator rather than filed, per the no-backlog-store
...
- `guard_audit_reconcile.py` now completes in ~55s and emits 39 real `[id] PASS/FAIL` markers
  (previously: `[audit.no-criteria] FAIL` immediately; after fixing 1–2 but before 3:
  non-terminating). The red/green pattern is coherent for an unstarted plan.
- `validate_demo.py` PASS (0 warnings); all 33 matrix ids proven by ≥1 beat.
...
7. **The negative-control criteria cannot fail closed when their mutation cannot be applied.**
   The audit harness ignores the `mutate` step's exit code, so a control whose artifact does
   not yet exist reports PASS having proven nothing (`browser-cjs-umd-repair.6` does this
   today). The mutate scripts fail loudly and refuse to write, but the harness does not read
