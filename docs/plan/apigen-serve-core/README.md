...
- `[dod.14]` **The mount-op Transport-stamping mechanism is resolved AND tested in Phase 1: OpPlan carries a transport field stamped per-package, and dispatchForPlan's mount branch adapts runtime Call -> core-client Call stamping Call.transport from plan.transport (never a hardcoded 'http' that would mis-tag mcp/cli mount provenance). (structural)** — The mount-op Transport-stamping mechanism is resolved AND tested in Phase 1: OpPlan carries a transport field stamped per-package, and dispatchForPlan's mount branch adapts runtime Call -> core-client Call stamping Call.transport from plan.transport (never a hardcoded 'http' that would mis-tag mcp/cli mount provenance)..

- `[dod.15]` **MCP's SSE and streaming-http HTTP-serving transports are preserved AND hardened under the TransportAdapter rewrite: the SSE GET /sse handshake + POST /messages session routing + onclose cleanup + no-start()-twice-crash invariant hold, and the streaming-http handler gains the error guard it lacks today (BUG-APIGEN-MCP-STREAMING-HTTP-NO-ERROR-GUARD-001, run.ts:269-276) so a handler rejection can never tear down the server — both driven by REAL clients with a negative control. (behavioral)** — MCP's SSE and streaming-http HTTP-serving transports are preserved AND hardened under the TransportAdapter rewrite: the SSE GET /sse handshake + POST /messages session routing + onclose cleanup + no-start()-twice-crash invariant hold, and the streaming-http handler gains the error guard it lacks today (BUG-APIGEN-MCP-STREAMING-HTTP-NO-ERROR-GUARD-001, run.ts:269-276) so a handler rejection can never tear down the server — both driven by REAL clients with a negative control..
  - given: <preconditions the consumer is in>
  - when: <the consumer performs the interaction>
  - then: <the consumer observes the result that proves success>
  - entrypoint: `CI=true ./node_modules/.bin/nx run apigen-plugin-mcp:test`
  - observable: `a real SSE client completes the /sse handshake and a sessionId-routed /messages POST; a handler error returns an error response WITHOUT teardown (a subsequent request on the same server still succeeds); the streaming-http handler catches errors (no unhandled rejection / process crash)`
  - negative-control: `git apply docs/plan/apigen-serve-core/neg-control/mcp-adapter.patch (removes the streaming-http error guard / breaks the /sse handshake) must flip CI=true ./node_modules/.bin/nx run apigen-plugin-mcp:test RED; git apply -R restores GREEN`
  - delivered-by: `mcp-adapter`
