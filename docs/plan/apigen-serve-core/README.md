...
- `[dod.10]` **No regressions: CI=true npx nx affected -t test is green across all affected apigen packages, and verify-dist-load is green for every affected package a consumer loads dist from. (structural)** — No regressions: CI=true npx nx affected -t test is green across all affected apigen packages, and verify-dist-load is green for every affected package a consumer loads dist from..
