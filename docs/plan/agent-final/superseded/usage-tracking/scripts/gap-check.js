...
function main() {
  ...
  if (!planDir) {
    process.stderr.write("usage: node scripts/gap-check.js <plan-dir> [--discover]\n");
    process.exit(2);
  }
  ...
  const boundary = discover
    ? "NOTE: --discover ran (caller mapping checked against the declared `changes` set). The judgment review in references/gap-review.md still requires human eyes."
    : "NOTE: mechanical checks only — Step-0 caller mapping (re-run with --discover) and the judgment review (references/gap-review.md) are NOT verified by this script.";
