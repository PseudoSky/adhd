...
### OBS-28 — intent-mining B: the owner's misses mapped; the compilation mechanism is SHIPPED and the demo never cited it `[miner-B 2026-07-17, spot-check pending]`
Master finding: the five named misses split into **(A) genuinely unbuilt design** and **(B) shipped code absent from the demo's narrative** — conflating them was the demo's core failure. The 26-row capability map (full table in the miner report, to be committed as the intent matrix with the rebuild):
- **"Intelligent task-based SP compilation" is BUILT:** `compileAgent({agentSlug, platform, context})` with the `(agent_slug, context_hash)` `composed_prompts` cache is shipped, live-wired at session start (agent-mcp-refactor, all compiler-integration states real at code level; cache-reuse proven by DB reopen per its `session-e2e.md:9-31`), resting on the shipped `context_condition` junction (`agent-registry-schema` `composition-junction.md:8-49`). **Zero hits for `compileAgent` in demo/DEMO.md** — the cheapest, highest-leverage fix is narrative, not engineering.
- **"Client as a generic package" has TWO designed referents, neither built, one previously unstubbed:** `@adhd/agent-runtime` (`RuntimeClient`+`UsageClient`, agent-mcp-refactor D6 addendum `decisions.md:165-211` — package does not exist) and `createAgentEngineClient()` (arch §3b, already stubbed U6). #11 had **no stub treatment at all** — missing outright, not marked missing.
- **"Dispatcher user interactions" includes a named, fully-spec'd UX with zero demo coverage:** the **Observable Agent Inspector CLI** — `agent-mcp inspect` / `agents` / `trace <run-id>` / `policy explain <agent-id>` with a full traced-run JSON schema (`docs/agent-mcp/agent-mcp-recommendation-evidence-requirements.md:152-209`). Plus the empirically-validated **capability-tiered multi-agent dispatch** pattern (code-tasking study: 14B lead reliably dispatching synth→coder→compose, `LOG.md:106-113`; task/role-conditioned prompts materially change capability, `README.md:118-144`).
- **"Graph components" has two referents:** primary — components as `sox-graph-store` nodes fused by `sox-hybrid-search` (agent-mcp-authoring D6, unbuilt); secondary — **GitNexus blast-radius signal feeding dispatch packing** (`dispatch-plugin-gitnexus` reusing this repo's own `gitnexus_impact`/`gitnexus_context`, `dispatch-completion/TOOLS.md:60` — shipped-side design, demo covers via DSP rows).
- **Dispatch residual gaps in the demo:** `stepwise-dispatch` A/B (op-granular + `ForwardContext`, `USE_CASES.md:63-72`) and `ICalibrationStore` — zero coverage.
- Reconfirmed: agents-as-cache (refactor D1) marked complete but contradicted by live source (I-12 family) — must re-verify before any demo claims it; policy stays owner-held (O-1); the task-runtime trio (`depends_on` DAG, schema, parallel tools) is shipped and demoable **today** and is NOT what "task-based compilation" means — different altitude.
- **Ecosystem generalization of graph+RAG:** two-stage retrieval (find the package family → pick within it) driven by one package graph (`adhd-agent-package-family-discovery-plan.md:580-682`) — the value case beyond agent components.

### OBS-29 — intent-mining A: the namesake gap, Domain 4, and four shipped-but-silently-unowned capabilities `[miner-A 2026-07-17, grep-verified against demo/GOAL]`
- **Largest gap in the corpus: corpus-scale migration** (`agent-registry-migration`) — the 346-agent ingest is the initiative's namesake and has **zero** demo trace (`importCorpus`/`corpus`/`346`: 0 hits): deterministic 18-type parser (nothing silently dropped, `unmapped[]`), two-tier LLM semantic decomposition (haiku fan-out → sonnet consolidation into a canonical use-case vocabulary), durable dataset build, public `importCorpus()` lib+CLI, and the **round-trip equivalence gate** (compile-and-diff vs source; `retire()` refuses deletion on any FAIL). The current demo's 3-component fixture proves none of it.
- **Domain 4 (Workflow Structures) is fully designed + seeded and has never received a disposition:** Playbooks (versioned multi-phase agent sequences w/ gates), Runbooks (scenario overrides of a playbook), Deployment Modes (active-agent subsets the supervisor reads), Strategy/Phases (promotion gates as DB rows) — `DATA_MODEL.md:309-345`, seeds `SEED_DATA.md:976-1025`, package owner "TBD," absent from COVERAGE (SYNTHESIS:140-141). Unlike policy (O-1) and dispatch (O-3), no ruling exists.
- **Shipped but zero demo coverage (silent drops, need explicit rulings per miner recommendation):** HITL suspend/resume (`request_human_input` → `awaiting_input` → durable `resumeToken` → `task_resume`; shipped agent-mcp@0.3.0, excluded from `dispatchReady()`) · SSE task streaming (`GET /tasks/:id/stream`, `stream:true` → `stream_url`; shipped @0.4.0) · usage delegation rollup (`usage.direct` vs `usage.subtree` + zero-context multi-hop proof; demo only asserts `tokens > 0`) · **mandatory substrate-vocabulary discovery** (`prompt_types_list`/`tool_list`/`model_list`/`policy_list`/`usecase_list` — SPEC:342-346: "remove any and the agent is blind… composition becomes impossible"; 0 demo hits) · version pin/rollback.
- **OBS-27's authoring fork largely resolves in-plan:** `SPEC_AGENT_MCP_TOOL_INTERFACE.md:111-200,216-231` (the later, refined half of the same plan dir) specifies the two-tool `component_define`/`agent_define` + `component_search` surface — i.e. the AGENT_MCP_TOOL_INTERFACE.md composite/CLI-only design was the earlier draft whose §Decisions were never ratified; the SPEC + Plan 8 agree. Residual: the interface doc's operator-CLI lane (compile-to-.md, tool_grant/model_bind CLI) remains its own intent.
- Also: compiler CLI bin (`agent-registry compile <slug>`) is a headline DoD never shelled in the demo (MCP-only path used) · credentialing plan self-declares superseded by `docs/mcp-env/SPEC.md` (residual: registry seed rows + LM-Studio live proof) · governance delegation targets a package that doesn't exist (needs fresh disposition, out of agent-final scope) · `send_message`/`ToolPlugin` (agents-full-workflow) proposed, never shipped, no surface trace.

### OBS-30 — dispositions round + the three-surface architecture ruling `[direct: owner, 2026-07-17]`
1. **Corpus migration: IN, featured act** — the 346-agent `importCorpus` journey (parse → two-tier decomposition → search over the real corpus → equivalence gate → guarded retire).
2. **Domain 4:** owner doesn't recognize it ("I dont really know what this is") — **no ruling**; stays deferred-pending-explanation. But a NEW framing extracted from the same note: **the ingestion tool is a user-facing capability — "a way for users to automatically ingest their specs"** — not merely a one-time migration. The demo presents importCorpus as spec-ingestion for any user corpus.
3. **All six shipped-but-unowned capabilities: IN** (HITL, SSE + usage.direct/subtree, vocab tools + version pin, task-DAG trio), with two shaping notes: **the dispatch demo must thoroughly demonstrate its better DAG implementation**, and **HITL is implicitly demonstrated through the dispatcher's Q&A system** (the adhd-build lineage Q&A — blocking/non-blocking questions — riding on `request_human_input`/`task_resume`).
4. **All four unbuilt-UX items: IN as ⟦U⟧-stubbed build scope** (Inspector surface, capability-tiered dispatch journey, the generic runtime client package, stepwise-dispatch A/B) — with **package names conforming to the standard: `@adhd/<group>-<layer>-<package>`** (so "agent-runtime at packages/shared/" is dead-on-arrival naming; the real name is assigned at plan time, e.g. `agent-core-runtime`).
5. **Three-surface architecture (new, load-bearing):** every package exposes a **clearly defined TS client interface**; the demo must show **third-party direct usage** of those clients (import-and-call), and how the SAME clients surface **differently via MCP (for agents) vs CLI (for humans)**. **Exactly two CLIs:** ONE **agent CLI** — the human counterpart of agent-mcp, covering ALL agent client operations (authoring, configuring, running, inspecting) — and ONE **dispatch CLI** (dispatch operations, per the retained demo). Consequences: the compiler CLI bin, the operator lane (compile-to-.md, browsing), the Inspector (`inspect`/`trace`/`agents`), and `importCorpus` all become subcommands of the one agent CLI rather than scattered binaries; Act 3 (client-as-package) expands to client → {3rd-party import, MCP surface, CLI surface} triplets.
6. **Directive: Q&A over all conflicting plan items, with timestamps of when each statement was written** — register below, first round asked 2026-07-17.

### OBS-31 — conflicts Q&A round 1 rulings `[direct: owner, 2026-07-17]`
- **C1 (MCP operator boundary): full surface parity — "restriction, not amputation."** Verbatim: "We should be able to restrict what agents have access to within the system when complete — so there is no reason to limit the support surface." Everything the agent CLI can do exists on MCP too; per-agent access restriction is a future system capability (note: this is the first concrete USE CASE for the O-1 policy/restrictions intent — access control over the system's own surface — without reviving enforcement scope now).
- **C2 (agents-as-cache): NO ruling by fiat — replaced with a use case + a method directive.** The registry client must support **resolving an SP for a task**; what the dispatcher does with the prompt is the dispatcher's business. Cache skepticism stated as a lifecycle hypothesis: a session materializes its SP into the session log once — nothing should need to re-read it, so `composed_prompts` may have no consumer ("maybe I'm missing something"). **Method directive (governs everything from here): builds are driven by actual use cases, not statements — surface use cases, not requirements.** The cache's survival now hangs on a falsifiable use case (cross-session identical-compile reuse) rather than a design assertion.
- **C3 (identity): name everywhere; slug dies.** Registry re-keys on name (or slug demotes to a semantics-free internal column). The Plan 8 seam (`name-slug.ts`, `registry-bridge.ts`, `inv:no-slug-on-wire`) is never built. Sequencing consequence: the identity migration must land **before** the 346-agent corpus ingest multiplies the rows.
- **C4 (plan format): dag.json single-source direction, dispatch-native — with the stated aspiration** verbatim: "Ideally when we're done we wont need plan state machine anymore because the single dag system is so good." agent-final's own executable plan is authored in the dispatch dag format and run by the dispatcher (dogfood); design consequence: the dispatch dag system must absorb the authoring/audit affordances (gap-check/audit-gate equivalents) that plan-state-machine currently provides, or the aspiration fails on tooling.
