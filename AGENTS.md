...
- You file, claim, transition, and resolve backlog items via the `adhd-backlog` CLI (renamed from the bare `backlog` bin, which collided with the unrelated public npm package `backlog@1.4.56`) / `mcp__backlog__*` tools — never by hand-editing a `BACKLOG.md`. The graph is the source of truth and any `BACKLOG.md` still on disk is a dead artifact, not a projection: nothing regenerates it, so a hand edit is simply invisible to every reader. Identity is the `uid` each verb returns. The command surface, the `--input '<json>'` calling convention, and a worked example of every verb live in `entrypoint/backlog/skill/SKILL.md` — read it rather than guessing a flag. (For this repo, this supersedes the global "store deferrals/bugs to BACKLOG.md" disclosure rule.)
...
Generated output, scratch DBs, logs, and test fixtures are **ephemeral and must never be tracked or scattered**. Application-layer packages write under `tmp/<package>/…` (e.g. `tmp/apigen/generate-out`, `tmp/agent-mcp/test.db`), gitignored. The Nx **tooling layer** (`tools/nx-plugins/**`) instead uses `.adhd/tmp/` as its scratch root — see `tools/nx-plugins/lib/scratch-root.js`'s `scratchRootPath()`/`isScratchPath()`, the single canonical definition consumers there must import rather than hardcode; this includes the release-publish manifest backstop (`tools/nx-plugins/build/lib/release-manifest.js`). Both roots are gitignored, but **neither is cleaned by `nx reset`** (that only clears `.nx/cache`) **nor by any project `clean` target** (none exists) — deleting accumulated content under `tmp/` or `.adhd/tmp/` is a manual, deliberate operator action.
...
- **MUST run impact analysis before editing any symbol.** Before modifying a function, class, or method, run `gx impact <symbol>` (defaults to `--direction upstream`) and report the blast radius (direct callers, affected processes, risk level) to the user.
- **MUST run `gx raw detect-changes` before committing** to verify your changes only affect expected symbols and execution flows.
- **MUST warn the user** if impact analysis returns HIGH or CRITICAL risk before proceeding with edits.
- When exploring unfamiliar code, use `gx query "<concept>"` to find execution flows instead of grepping. It returns process-grouped results ranked by relevance.
- When you need full context on a specific symbol — callers, callees, which execution flows it participates in — use `gx context <symbol>`.

## Never Do

- NEVER edit a function, class, or method without first running `gx impact` on it.
- NEVER ignore HIGH or CRITICAL risk warnings from impact analysis.
- NEVER rename symbols with find-and-replace — use `gx raw rename` which understands the call graph.
- NEVER commit changes without running `gx raw detect-changes` to check affected scope.
