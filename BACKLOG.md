...
### BUG-NXTEST-001 — 75 test cases can never run: 3 projects ship specs with no `test` target, and `nx run-many -t test` reports **SUCCESS** for them
- **Discovered:** 2026-07-16, while inventorying the agent+dispatch subsystems to author the agent-final demo set.
- **Symptom:** three projects contain real, substantial spec files but declare **no `test` target** in `project.json`. Nx does not error on a missing target requested via `run-many` — it reports success and exits 0. So the specs are invisible: they never run, never fail, and never appear in any count.

| Project | Spec file | Lines | Test cases | `test` target |
|---|---|---|---|---|
| `dispatch-core-optimizer` | `src/lib/optimize.spec.ts` | 280 | 12 | ❌ |
| `dispatch-core-optimizer` | `src/lib/snapshot.spec.ts` | 361 | 18 | ❌ |
| `agent-plugin-budget` | `src/__tests__/budget-plugin.test.ts` | 1404 | 35 | ❌ |
| `agent-plugin-sanitize` | `src/__tests__/sanitize-plugin.test.ts` | 193 | 10 | ❌ |
| | | | **75** | |

- **Proof (mechanical, reproducible):**
  ```
  $ npx nx run-many -t test -p dispatch-core-optimizer,agent-plugin-budget
    NX   Successfully ran target test for 2 projects and 4 tasks they depend on
  EXIT=0
  ```
  Neither project has a `test` target. Nx reports having run it for both. Exit 0.
- **Why it matters — this has already produced false "green" claims in three published documents:**
  1. `BACKLOG.md:23` (BUG-DISPATCH-PUBLISH-001 closure evidence) — *"Verified green: `npx nx run-many -t build test --projects=dispatch-base-spec,dispatch-base-types,dispatch-core-client,dispatch-core-optimizer,…` → EXIT 0, all tasks passed (30/30 dispatch-cli tests)"*. That invocation **named `dispatch-core-optimizer` and `dispatch-base-types`**, both of which have no `test` target. The "all tasks passed" is partly vacuous; the 30/30 is `dispatch-cli` alone.
  2. `docs/architecture/agent-dispatch-systems.md:268` §4 *"What is REAL today (do not rebuild)"* — *"Dispatch's snapshot/optimize/serialize/CLI spine — builds green, 30 tests pass."* The optimizer's own 30 test cases (12+18) are not among them. `optimize()` is the reason the dispatch subsystem exists (`docs/plan/agent-final/superseded/dispatch-completion/demo/DEMO.md:219`, "Act 3 — Pack the cheapest next wave … This is the reason the subsystem exists").
  3. `docs/architecture/agent-dispatch-systems.md:267` — *"The plugin seam (`loader.ts:260-281`) — proven live by budget."* `agent-plugin-budget`'s 35 test cases (1404 lines — the largest orphaned suite here) never execute via any nx target. The seam may well work; **this sentence is not the evidence that it does.**
- **Class:** exactly the `green-but-untyped/unrun` failure mode that `docs/plan/agent-final/superseded/agent-registry/DEMO.md:26-28` was written to kill (*"The package shipped '39/39 green' while `nx typecheck` was red (13 errors hidden because vitest doesn't type-check)"*). Same root shape: a target that isn't wired reports nothing rather than failing. Sibling of `DEBT-DISPATCH-018` (`docs/plan/dispatch-production/dag.json` guards invoke a `typecheck` target no project defines → "Cannot find configuration for task", unrunnable as written) — that one at least errors; this one lies.
- **Fix direction:** add a `test` target to `dispatch-core-optimizer`, `agent-plugin-budget`, `agent-plugin-sanitize` (mirror the `@nx/vite:test` target their siblings already declare, e.g. `packages/dispatch/dispatch-core-client/project.json`), then **run them and fix whatever is red** — these 75 cases have never executed and must not be assumed green. Additionally: add a repo-level gate asserting that every project containing a `*.spec.ts`/`*.test.ts` under `src/` declares a `test` target (the same shape as the existing `scripts/check-publish-hygiene.mjs` gate), so `run-many`'s silent-success can never manufacture a false green again.
- **Note:** `dispatch-base-types` also has 1 spec + no `test` target, but it is dead Nx scaffold (`src/lib/dispatch-dispatch-base-types.ts:1-3` — never customized past generator boilerplate) already tracked for deletion. **That tracking is now stranded**: `BACKLOG.md:25` defers it to `docs/plan/dispatch-completion`, which was quarantined into `docs/plan/agent-final/superseded/dispatch-completion/` on 2026-07-16 — see CHORE-REPO-003 below.
- **Status:** OPEN.
- Citations: [main @ 65f3707d, workflow:plan-builder, claude, agent-final demo-set inventory, 1: packages/dispatch/dispatch-core-optimizer/project.json, 2: packages/dispatch/dispatch-core-optimizer/src/lib/optimize.spec.ts, 3: packages/dispatch/dispatch-core-optimizer/src/lib/snapshot.spec.ts, 4: packages/agent/agent-plugin-budget/project.json, 5: packages/agent/agent-plugin-budget/src/__tests__/budget-plugin.test.ts, 6: packages/agent/agent-plugin-sanitize/project.json, 7: packages/agent/agent-plugin-sanitize/src/__tests__/sanitize-plugin.test.ts, 8: BACKLOG.md:23, 9: docs/architecture/agent-dispatch-systems.md:267-268, 10: docs/plan/agent-final/superseded/agent-registry/DEMO.md:26-28, 11: packages/dispatch/dispatch-base-types/src/lib/dispatch-dispatch-base-types.ts:1-3]
