...
- **Status:** OPEN — no hook-config change made this session (out of scope for BUG-APIGEN-033; the incident was resolved via a follow-up commit, not a hook fix). Whoever owns the shared pre-commit hook config should pick this up before the next concurrent-agent collision.
...
- **Leading hypothesis (mechanism — UNPROVEN, for the hook owner to confirm):** in a worktree the `node_modules` symlink / tsconfig-path resolution is off (known quirk — see the worktree cache/base-ref debt), so `@nx/dependency-checks` cannot resolve the source imports, concludes the deps are obsolete, and `--fix` deletes them. If true, EVERY `--fix` commit made inside a worktree is at risk of silently stripping used deps. Needs confirmation: does the same `nx lint --fix` on main (non-worktree) preserve these deps? (If it strips them on main too, the rule/config is simply misanalyzing imports and the severity is repo-wide.)
- **Fix direction:** (a) the strongest is to REMOVE `--fix` from the commit path entirely (make the hook a verifier, not a mutator — already the direction of the dep-sync redesign); (b) the `verify-dist-load` gate + a `verify-deps-resolvable` check (npm pack dry-run / actually resolve each declared+imported dep) must run before publish so a stripped dep can never ship; (c) audit `@nx/dependency-checks` config for why static imports go undetected (worktree resolution, `ignoredFiles`, tsconfig `paths` context) and never run its `--fix` in a context where module resolution is unreliable. Belongs to whoever owns the hook/dep-sync redesign (currently in flight).
- **Cross-references:** BUG-REPO-PRECOMMIT-PARTIAL-STAGE-001 (same hook, full-file re-stage), DEBT-WORKSPACE-DEPCHECK-001 (why the rule was enabled), DEBT-NX-WORKTREE-CACHE-001 (worktree resolution quirk), the apigen build-vs-ship dist-load gap, DEBT-DISPATCH-PUBLISH-GATE-001 (npm pack dep-resolvability gate).
- **Status:** OPEN — evidence captured; not fixed. Forwarded to the in-flight dep-sync/hook redesign agent.
