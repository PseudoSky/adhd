...
**Baseline CORRECTED (2026-07-20):** deleted the 41 wrong tags; **kept only the 11 already-published tags** (at their exact published versions). Re-proven via bare `nx release version --dry-run`: **11 resolve-from-tag → `🚫 Skipping`; 41 (no tag) → selected for release** (fall back to disk, conventional-commits bump). This is the honest bootstrap: "these 11 are on npm; the other 41 are unpublished and ship on the next release." (Surfaced BUILD-CONSIST-011 en route: `data-*`/`ui-react-base-hooks` dist is a version ahead of source — fix before releasing those three.)
**Remaining before a real publish (genuine human/policy decisions):** (1) **First publish of the 41 — pick the target version policy:** bare `nx release` bumps them via conventional-commits from full history (e.g. `dispatch-serializer-json 0.0.1→0.1.0`), OR `nx release --first-release` ships their *current disk* versions as-is. (2) **Fix BUILD-CONSIST-011** (dist>source skew on 3 packages) before including them. (3) **push tags** (`git push --tags`, human-approved) so CI sees changed-only. (4) one real `nx release publish --dry-run` end-to-end once (1)-(3) settled.
...
### BUILD-CONSIST-011 — dist is a version AHEAD of source for `data-base-transforms`, `data-query-engine`, `ui-react-base-hooks` (source 2.2.1, dist 2.2.2)
- **Discovered:** 2026-07-20, bootstrapping `nx release` baseline tags — nx release reads the version "from `dist/…/package.json`" and reported `2.2.2` for these three while their **source** `package.json` says `2.2.1`. Verified directly: `packages/data/data-base-transforms`, `packages/data/data-query-engine`, `packages/ui-react/ui-react-base-hooks` all show `source=2.2.1 dist=2.2.2`[1].
- **Impact:** `nx release version`/`publish` operate on the **dist** version, so a release would treat these as `2.2.2` even though source (the source of truth, and what the nx cache hashes via the `production` input) is `2.2.1`. None of the three is published on npm (404), so no "cannot-publish-over-existing" break yet — but the skew means a publish would ship `2.2.2` built from `2.2.1` source, and the two can silently diverge further. This is the "stale dist over fresh source" class the AGENTS.md `--skip-nx-cache` warning describes.
- **Root cause (CONFIRMED 2026-07-20):** source has been `2.2.1` since commit `faaddc56` (bumped 0.0.1→2.2.1) and was **never `2.2.2`** in git history[2] — so `2.2.2` did not come from source. It lives in the **cached `build` task output** (Nx 18.3.4 file cache, `.nx/cache/<hash>/`): deleting the 3 `dist/` dirs and re-running `nx build` **restores `2.2.2` from cache** ("read the output from the cache … 6 of 6 tasks")[3], and a plain rebuild is a cache hit. Almost certainly a prior real `nx release version` (which writes the bumped version into `dist/.../package.json`) got captured into the build cache. npm has neither version (404), so nothing shipped.
- **Fix direction (heavier than a plain rebuild — plain rebuild & dist-deletion both FAIL, cache restores 2.2.2):** must evict the poisoned cache — either `npx nx reset` (clears the **entire** workspace cache → team-wide recompute; deliberate, recoverable) then `npx nx build …` the three, OR surgically remove their `.nx/cache/<hash>` entries. **Deferred, not done mid-session:** wiping the shared cache while concurrent agents build is high-blast, and these three are unpublished (pending the release decision in DEBT-RELEASE-BASELINE-TAGS-001), so the reset should be run **deliberately, immediately before releasing these three**, and `source==dist` re-verified then.
- **Status:** OPEN — root cause confirmed; fix (cache eviction) deferred to release-time by design. Citations: [main@088266df, claude, release-verification, 1: source vs dist `package.json` version; 2: `git log -L` on the version line (source never 2.2.2, 2.2.1 since faaddc56); 3: `rm -rf dist/{…3 dirs}` + `nx run-many -t build` → cache-hit restored 2.2.2, 2026-07-20]
