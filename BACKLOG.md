...
**Remaining before a real publish:** (1) **decide whether the current disk versions are already on npm** — the local baseline tags assert "already released," so if e.g. `agent-mcp@2.1.1` is NOT yet published, a bare release will *skip* it (ships 2.1.2 next change, never 2.1.1); if bootstrapping should instead PUBLISH current versions, use `nx release --first-release`. (2) **push the baseline tags** (human-approved) so CI sees changed-only. (3) one real `nx release publish --dry-run` end-to-end once (1)/(2) are settled.
**Status:** PARTIALLY RESOLVED — changed-only detection is functional and proven locally; publish is gated on the human decisions in *Remaining* above (versions-already-published? + push tags). No longer blocks the "build fixes" claim (those are separately proven green).
