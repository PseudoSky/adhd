...
### INVESTIGATION-BUILD-TOOL-001 — `@nx/vite:build` with `external: []` on `platform:node` packages bundles transitive TypeScript deps that crash on dynamic import; the tsc/vite split needs a per-project audit
- **Discovered:** 2026-07-20, while triaging BUG-APIGEN-041 (plugin dist crashes when dynamically imported via `--use <path>`).
- **Evidence:** `apigen-plugin-openapi` (`platform:node`, publishing-only deps) builds with `@nx/vite:build` and `external: []`. This causes Rollup to follow the entire transitive import chain from `@adhd/apigen-core-client` → `ts-morph` → `typescript` → `perf_hooks` and inline everything into a 9.8MB bundle. TypeScript's module-init code (`zj()` → `performance.timeOrigin`) crashes on evaluation because `@rollup/plugin-commonjs` generates broken stubs for `require('perf_hooks')`. The plugin's own code path never calls ts-morph or TypeScript — they're bundled solely because Vite follows all imports through barrel exports. Reproduced identically across `apigen-codegen-openapi`, `apigen-plugin-openapi`, and `apigen-engine-conformance` dists (all use `@nx/vite:build` with `external: []`).
- **Challenges BUILD-CONSIST-008:** That item concluded the tsc/vite split is "principled but undocumented" and that `platform:node` packages without native addon deps "correctly use `@nx/vite:build`". This investigation found that `@nx/vite:build` with `external: []` produces a broken artifact for ANY package whose transitive dep graph includes TypeScript — which is nearly every apigen package (through `@adhd/apigen-core-client` → `ts-morph`). BUILD-CONSIST-008's analysis checked for native addon dependencies but did not account for `ts-morph`/`typescript`'s CJS-interop fragility under Rollup.
- **Scope of investigation needed (per-project):**
  1. For each `platform:node` package using `@nx/vite:build` — does `@nx/js:tsc` work? (It preserves `require()` chains, no Rollup stubs, dynamic `import()` works.)
  2. For each `platform:shared` package using `@nx/vite:build` — does it actually need to run in a browser? If not, `@nx/js:tsc` is simpler. If yes, the `external` array must explicitly exclude `ts-morph`/`ts-json-schema-generator`/`typescript` (they can't run in a browser anyway, browser-bundling is dead code).
  3. For any package where `@nx/vite:build` is genuinely necessary (e.g. `platform:browser` components), verify that `commonjsOptions` and `external` are configured correctly and the dist actually loads without crashing.
- **Not prescribing a fix — this is a devops investigation.** The answer may be "use `@nx/js:tsc` for all `platform:node` packages" or "keep `@nx/vite:build` but fix the `external` config per package." Either path needs a per-package audit and a documented rationale per `project.json`.
- **Generator enforcement:** Whatever decision is reached, `@adhd/workspace-codegen-nx` generators must be updated so newly scaffolded packages automatically get the correct build executor (`@nx/js:tsc` vs `@nx/vite:build`) based on their `platform` tag. This prevents future packages from inheriting the wrong build config by convention/mistake, the way `apigen-plugin-openapi` did.
- **Cross-references:** BUILD-CONSIST-008 (existing tsc/vite analysis, challenged by this finding), BUG-APIGEN-041 (the triggering incident), DEBT-APIGEN-SCHEMA-ENUM-001 (another apigen codegen quality issue discovered in the same investigation session), `packages/apigen/apigen-plugin-openapi/vite.config.ts` (the config whose `external: []` caused the crash).
- **Status:** OPEN — investigation findings documented, no fix applied.
