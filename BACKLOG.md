...
- **Root cause (likely):** dist was built when source was `2.2.2`, then source was reverted/renumbered to `2.2.1` without a rebuild, OR a prior version write hit dist only. Not yet root-caused to a specific commit.
- **Fix direction:** `npx nx reset` then `npx nx build data-base-transforms data-query-engine ui-react-base-hooks` (or run-many) to rebuild dist from the current `2.2.1` source and confirm `source==dist`; then decide the intended release version deliberately. **Do this before any release that includes these three.**
- **Status:** OPEN. Citations: [main@f6f1d8bd, claude, release-verification, 1: `packages/{data/data-base-transforms,data/data-query-engine,ui-react/ui-react-base-hooks}/package.json` version vs `dist/…/package.json` version, 2026-07-20]
